-- English V2: one Central Intelligence, one concept evidence model, multiple delivery routes.
-- This migration turns Add Context / I Guessed into durable repair signals without creating a parallel mastery system.
...
create table if not exists english.learner_confusions(
  ...
  status text not null default 'open' check(status in ('open','testing','improving','resolved','reopened')),
...
create or replace function english.log_learning_activity(
  ...
  p_concept_id text default null,p_source_note_id uuid default null,p_route text default null,
...
begin
  insert into english.learning_intelligence_activity(
    ...
    p_user_id,left(coalesce(nullif(trim(p_type),''),'learning_action'),80),
    left(coalesce(nullif(trim(p_title),''),'Learning action'),180),nullif(left(coalesce(p_detail,''),600),''),
    p_question_id,p_concept_id,p_source_note_id,p_route,coalesce(p_metadata,'{}'::jsonb),coalesce(p_occurred_at,now())
...
begin
  ...
  insert into english.learner_confusions(
    ...
    strength=least(99,english.learner_confusions.strength+1),last_signal_at=now(),resolved_at=null,
    metadata=english.learner_confusions.metadata||excluded.metadata,updated_at=now()
...
begin
  ...
  into related_ids,related_words from(
    ...
    where q.active and q.question_id<>n.question_id and english.question_visible_to_user(p_user_id,q.question_id)
      ...
      and position(lower(trim(q.word)) in lower(n.note))>0 and (cid is null or qm.concept_id is distinct from cid)
...
-- Confidence now reflects unresolved learner uncertainty, not permanent historical punishment.
...
begin
  with raw as(
    select a.attempted_at occurred_at,a.correct,a.question_id,coalesce(nullif(a.module,''),'practice') variant,a.question_id evidence_key
    ...
    union all
    select e.occurred_at,e.correct,e.question_id,coalesce(nullif(e.variant,''),e.source) variant,coalesce(e.question_id,e.source_key) evidence_key
  ...
  select count(*)::int attempts,count(*) filter(where correct is true)::int correct,count(*) filter(where correct is false)::int wrong,
    ...
    count(*) filter(where correct is false and occurred_at>=now()-interval '7 days')::int recent_failures,max(occurred_at) last_attempt_at
  into r from ordered;
  ...
  where c.user_id=p_user_id and c.status<>'resolved' and (c.primary_concept_id=p_concept_id or c.related_concept_id=p_concept_id);
  ...
  score:=greatest(0,least(100,
    ...
    -least(30,coalesce(r.recent_failures,0)*12)-least(20,v_guessed*5)-least(20,v_confusions*7)
  ));
  state:=case when coalesce(r.attempts,0)=0 then 'unseen'
    ...
    when score>=60 and coalesce(r.distinct_questions,0)>=2 then 'secure' else 'seen' end;
...
set search_path to 'pg_catalog','english'
as $$
declare cid text; c record; correct_q int; wrong_n int; first_ok timestamptz; last_ok timestamptz; previous_status text; rr record;
begin
  ...
  if coalesce(new.correct,false) then
    ...
    where g.user_id=new.user_id and g.signal='guessed' and g.resolved_at is null and g.created_at<new.attempted_at
      and g.question_id<>new.question_id and exists(
        select 1 from english.question_concept_mappings m where m.question_id=g.question_id and m.concept_id=cid);
  end if;
  ...
  loop
    ...
    into correct_q,wrong_n,first_ok,last_ok
    from english.attempts a join english.question_concept_mappings m on m.question_id=a.question_id
    ...
    if coalesce(wrong_n,0)>0 then
    ...
    elsif coalesce(correct_q,0)>=2 and first_ok is not null and last_ok-first_ok>=interval '20 hours' then
      ...
      where confusion_id=c.confusion_id;
      if previous_status<>'resolved' then perform english.log_learning_activity(new.user_id,'confusion_resolved','Confusion resolved',
        coalesce(nullif(c.related_term,''),'Confusion pair')||' · fresh and spaced validation passed',new.question_id,c.primary_concept_id,
        c.source_note_id,'targeted',jsonb_build_object('confusion_id',c.confusion_id),new.attempted_at); end if;
    elsif coalesce(correct_q,0)>=2 then
      update english.learner_confusions set status='improving',last_validation_at=new.attempted_at,updated_at=now() where confusion_id=c.confusion_id;
    elsif coalesce(correct_q,0)>=1 then
      update english.learner_confusions set status='testing',last_validation_at=new.attempted_at,updated_at=now() where confusion_id=c.confusion_id;
    end if;
  end loop;

  if coalesce(new.correct,false) then
    for rr in select lr.* from english.learning_route_state lr join english.question_concept_mappings m on m.question_id=lr.question_id
    ...
    loop
      ...
      where a.user_id=new.user_id and am.concept_id=cid and a.attempted_at>=coalesce(rr.targeted_at,'epoch'::timestamptz);

      if coalesce(wrong_n,0)=0 and coalesce(correct_q,0)>=2 and first_ok is not null and last_ok-first_ok>=interval '20 hours'
        and not exists(select 1 from english.learner_confusions c2 where c2.user_id=new.user_id and c2.status<>'resolved'
          and (c2.primary_concept_id=cid or c2.related_concept_id=cid))
        and not exists(select 1 from english.learner_confidence_signals g join english.question_concept_mappings gm on gm.question_id=g.question_id
          where g.user_id=new.user_id and gm.concept_id=cid and g.signal='guessed' and g.resolved_at is null)
      then
        perform english.route_to_fast_track(new.user_id,rr.question_id,'Recovered Targeted',
          'Targeted concept recovered with fresh transfer + spaced validation',true);
      elsif coalesce(wrong_n,0)=0 and coalesce(correct_q,0)>=2 then
        ...
        where user_id=new.user_id and question_id=rr.question_id and route='targeted';
      end if;
    end loop;
  end if;
...
-- I Guessed is a confidence signal. It preserves correctness and creates a transfer-check route.
...
as $$
declare uid uuid:=auth.uid(); cid text; aid text:=nullif(trim(coalesce(p_attempt_id,'')),''); inserted boolean:=false; alt_exists boolean:=false;
begin
  ...
  if aid is null then select attempt_id into aid from english.attempts where user_id=uid and question_id=p_question_id order by attempted_at desc,created_at desc limit 1; end if;
  ...
  elsif not exists(select 1 from english.learner_confidence_signals where user_id=uid and question_id=p_question_id and signal='guessed'
      and attempt_id is null and created_at>now()-interval '10 minutes') then
  ...
  if inserted then
    ...
    perform english.log_learning_activity(uid,'guess_transfer','I Guessed → transfer check',
      'Correctness is preserved; Central Intelligence will seek fresh validation.',p_question_id,cid,null,'targeted',jsonb_build_object('attempt_id',aid),now());
  end if;
  if cid is not null and inserted then perform english.recompute_concept_evidence(uid,cid); end if;
  select exists(select 1 from english.questions q join english.question_concept_mappings m on m.question_id=q.question_id
    where q.active and m.concept_id=cid and q.question_id<>p_question_id and english.question_visible_to_user(uid,q.question_id)) into alt_exists;
  return jsonb_build_object('ok',true,'signal','guessed','concept_id',cid,'attempt_id',aid,'recorded',inserted,
    'validation_due',case when inserted then now()+interval '12 hours' else null end,'alternate_available',alt_exists);
end $$;

-- Keep public compatibility wrapper and permissions.
create or replace function public.english_record_guess(p_question_id text,p_attempt_id text default null)
returns jsonb language sql security definer set search_path to 'pg_catalog','public','english','auth'
as $$ select english.english_record_guess(p_question_id,p_attempt_id); $$;
revoke all on function public.english_record_guess(text,text) from public,anon;
grant execute on function public.english_record_guess(text,text) to authenticated,service_role;

-- Targeted remains a delivery route. Future Daily batches reserve a small controlled share without rebuilding today's fixed batch.
create or replace function english.daily_reason_code(p_reason text) returns text language sql immutable
as $$ select case p_reason when 'Persistent Weak' then 'PW' when 'Targeted Repair' then 'TARGET' when 'Weak' then 'W'
  when 'Fragile' then 'FR' when 'Due Spaced Revision' then 'DUE' when 'Marked Review' then 'STAR'
  when 'Difficult Review' then 'DIFF' when 'Controlled New' then 'NEW' when 'Learning' then 'LEARN' else 'MIX' end; $$;
...
as $$ select greatest(1,floor(p_target*case p_reason when 'Persistent Weak' then .20 when 'Targeted Repair' then .10 when 'Weak' then .16
  when 'Fragile' then .14 when 'Due Spaced Revision' then .15 when 'Learning' then .08 when 'Marked Review' then .05
...
as $$ select greatest(1,ceil(p_target*case p_reason when 'Persistent Weak' then .30 when 'Targeted Repair' then .15 when 'Weak' then .25
  when 'Fragile' then .20 when 'Due Spaced Revision' then .25 when 'Learning' then .15 when 'Marked Review' then .10
...
with x as(
 select q,coalesce(s.attempts,0) attempts,coalesce(s.status,'New') state,s.next_review,coalesce(s.mastered,false) mastered,
   coalesce(s.last_marked,false) starred,coalesce(d.difficult,false) difficult,coalesce(r.route,'') route
 ...
 left join english.learning_route_state r on r.user_id=p_user_id and r.question_id=q.question_id where q.question_id=p_question_id
)
select case when not (q).active or mastered or route='fast_track' then '' when route='targeted' then 'Targeted Repair'
...
as $$
declare wanted int:=greatest(1,least(18,ceil(greatest(1,p_target)*.10)::int)); existing_n int; replace_n int;
begin
  select count(*) into existing_n from english.daily_current where user_id=p_user_id and quiz_date=p_batch_date and reason='Targeted Repair';
  replace_n:=greatest(0,wanted-existing_n); if replace_n=0 then return existing_n; end if;
  with targets as(
    select q.question_id,q.topic,m.concept_id,coalesce(ce.confidence_score,0) confidence,
      row_number() over(partition by coalesce(m.concept_id,q.question_id) order by
        case coalesce(r.metadata->>'targeted_kind','need_learning') when 'confusion' then 1 when 'need_learning' then 2 when 'transfer_check' then 3 else 4 end,
        coalesce(ce.next_review,'epoch'::timestamptz),r.updated_at desc,q.question_id) concept_pick
    from english.learning_route_state r join english.questions q on q.question_id=r.question_id and q.active
    left join english.question_concept_mappings m on m.question_id=q.question_id left join english.concept_evidence ce on ce.user_id=p_user_id and ce.concept_id=m.concept_id
    left join english.question_state s on s.user_id=p_user_id and s.question_id=q.question_id
    where r.user_id=p_user_id and r.route='targeted' and english.question_visible_to_user(p_user_id,q.question_id) and not coalesce(s.mastered,false)
      and not exists(select 1 from english.daily_current d left join english.question_concept_mappings dm on dm.question_id=d.question_id
        where d.user_id=p_user_id and d.quiz_date=p_batch_date and coalesce(dm.concept_id,d.question_id)=coalesce(m.concept_id,q.question_id))
      and not exists(select 1 from english.attempts a left join english.question_concept_mappings am on am.question_id=a.question_id
        where a.user_id=p_user_id and (a.attempted_at at time zone 'Asia/Kolkata')::date=p_batch_date
          and coalesce(am.concept_id,a.question_id)=coalesce(m.concept_id,q.question_id))
  ), pick as(
    select *,row_number() over(order by confidence asc,question_id) rn from targets where concept_pick=1 limit replace_n
  ), removable as(
    select d.question_id,d.sequence,row_number() over(order by case d.reason when 'Controlled New' then 1 when 'Learning' then 2
      when 'Marked Review' then 3 when 'Difficult Review' then 4 when 'Due Spaced Revision' then 5 when 'Fragile' then 6 when 'Weak' then 7 else 9 end,
      d.priority asc,d.sequence desc) rn
    from english.daily_current d where d.user_id=p_user_id and d.quiz_date=p_batch_date and d.reason<>'Targeted Repair'
      and lower(coalesce(d.status,''))<>'completed'
      and not exists(select 1 from english.attempts a where a.user_id=p_user_id and a.question_id=d.question_id
        and (a.attempted_at at time zone 'Asia/Kolkata')::date=p_batch_date)
    limit replace_n
  ), pairs as(select r.question_id old_id,r.sequence,p.* from removable r join pick p using(rn))
  update english.daily_current d set question_id=p.question_id,priority=950,reason='Targeted Repair',status='New',topic=p.topic,
    concept_id=p.concept_id,selection_signals=array['TARGET'],selection_snapshot=jsonb_build_object(
      'selectedAt',now(),'batchDate',p_batch_date,'reason','Targeted Repair','conceptId',p.concept_id,'conceptConfidence',p.confidence,'targeted',true)
  from pairs p where d.user_id=p_user_id and d.question_id=p.old_id and d.sequence=p.sequence;
  select count(*) into existing_n from english.daily_current where user_id=p_user_id and quiz_date=p_batch_date and reason='Targeted Repair';
  return existing_n;
end $$;
revoke all on function english.rebalance_daily_targeted(uuid,date,integer) from public,anon,authenticated;
grant execute on function english.rebalance_daily_targeted(uuid,date,integer) to service_role;

-- Preserve the proven selector and wrap only new-batch output.
alter function english.create_daily(uuid,date,integer) rename to create_daily_core_20260902;
create function english.create_daily(p_user_id uuid,p_batch_date date,p_target integer)
returns integer language plpgsql security definer set search_path to 'pg_catalog','english','auth'
as $$
declare n int;
begin
  n:=english.create_daily_core_20260902(p_user_id,p_batch_date,p_target);
  perform english.rebalance_daily_targeted(p_user_id,p_batch_date,p_target);
  select count(*) into n from english.daily_current where user_id=p_user_id and quiz_date=p_batch_date;
  return n;
end $$;
...
), dedup as(select t.*,row_number() over(partition by coalesce(t.concept_id,t.question_id) order by
 case t.kind when 'confusion' then 1 when 'need_learning' then 2 when 'transfer_check' then 3 when 'retention_check' then 4 else 5 end,t.updated_at desc) rn from targeted t),
base as(select * from dedup where rn=1), conf as(
 select cf.*,coalesce(c1.name,q1.word,cf.primary_concept_id) primary_name,
   coalesce(c2.name,q2.word,cf.related_term,cf.related_concept_id,'Related concept') related_name,left(coalesce(n.note,''),180) note
 ...
 join english.questions q on q.question_id=r.question_id left join english.question_concept_mappings m on m.question_id=q.question_id
 left join english.concepts c on c.concept_id=m.concept_id order by r.targeted_recovered_at desc limit 20
)
select case when uid.id is null then jsonb_build_object('ok',false,'error','Authentication required') else jsonb_build_object(
 ...
 'confusions',coalesce((select jsonb_agg(jsonb_build_object('confusionId',confusion_id,'status',status,'strength',strength,
   'primaryConceptId',primary_concept_id,'relatedConceptId',related_concept_id,'primaryQuestionId',primary_question_id,
   'relatedQuestionId',related_question_id,'primaryName',primary_name,'relatedName',related_name,'note',note,
   'openedAt',opened_at,'lastSignalAt',last_signal_at,'resolvedAt',resolved_at) order by last_signal_at desc)
   from conf where status<>'resolved'),'[]'::jsonb),
 'needLearning',coalesce((select jsonb_agg(jsonb_build_object('questionId',question_id,'conceptId',concept_id,'name',concept_name,
   'skillFamily',skill_family,'state',concept_state,'confidence',confidence,'reason',last_route_reason,'nextReview',next_review) order by updated_at desc)
   from(select * from base where kind='need_learning' order by updated_at desc limit 24)x),'[]'::jsonb),
 'transferChecks',coalesce((select jsonb_agg(jsonb_build_object('questionId',question_id,'conceptId',concept_id,'name',concept_name,
   'state',concept_state,'confidence',confidence,'reason',last_route_reason,'nextReview',next_review) order by updated_at desc)
   from(select * from base where kind='transfer_check' order by updated_at desc limit 24)x),'[]'::jsonb),
 'retentionChecks',coalesce((select jsonb_agg(jsonb_build_object('questionId',question_id,'conceptId',concept_id,'name',concept_name,
   'state',concept_state,'confidence',confidence,'reason',last_route_reason,'nextReview',next_review) order by coalesce(next_review,'epoch'::timestamptz))
   from(select * from base where kind='retention_check' order by coalesce(next_review,'epoch'::timestamptz) limit 24)x),'[]'::jsonb),
 'recovered',coalesce((select jsonb_agg(to_jsonb(z) order by z.at desc) from(
   ...
   union all select concept_id,concept_name,targeted_recovered_at,'targeted' from recovered order by at desc limit 24)z),'[]'::jsonb)
) end from uid;
$$;
...
as $$
declare uid uuid:=auth.uid(); n int:=greatest(1,least(30,coalesce(p_count,15))); k text:=lower(nullif(trim(coalesce(p_kind,'')),'')); outv jsonb;
begin
 if uid is null then raise exception 'Authentication required'; end if;
 with focus as(select * from english.learner_confusions where user_id=uid and p_confusion_id is not null and confusion_id=p_confusion_id),
 routed as(
 ...
 ), filtered as(select x.* from routed x where
   (p_confusion_id is null or exists(select 1 from focus f where x.concept_id=f.primary_concept_id or x.concept_id=f.related_concept_id
     or x.question_id=f.primary_question_id or x.question_id=f.related_question_id)) and (k is null or x.kind=k)),
 delivery as(
   select f.*,coalesce(alt.question_id,f.question_id) delivery_question_id,
     row_number() over(partition by coalesce(f.concept_id,f.question_id) order by case f.kind when 'confusion' then 1 when 'need_learning' then 2 when 'transfer_check' then 3 else 4 end,
 ...
 ), chosen as(select * from delivery where concept_pick=1 order by case kind when 'confusion' then 1 when 'need_learning' then 2 when 'transfer_check' then 3 else 4 end,
 ...
 select coalesce(jsonb_agg(english.question_payload(uid,c.delivery_question_id)||jsonb_build_object(
   ...
   'conceptId',c.concept_id,'conceptCoverage',coalesce(c.coverage_state,'unseen'),'conceptConfidence',coalesce(c.confidence_score,0),
   'conceptNextReview',c.next_review) order by case c.kind when 'confusion' then 1 when 'need_learning' then 2 when 'transfer_check' then 3 else 4 end,c.updated_at desc),'[]'::jsonb)
 into outv from chosen c; return outv;
end $$;
...
with uid as(select auth.uid() id), rows as(
 ...
 where (a.occurred_at at time zone 'Asia/Kolkata')::date=(now() at time zone 'Asia/Kolkata')::date order by a.occurred_at desc limit 20
)
select case when (select id from uid) is null then jsonb_build_object('ok',false,'error','Authentication required') else jsonb_build_object(
 'ok',true,'count',(select count(*) from rows),'items',coalesce((select jsonb_agg(jsonb_build_object(
   'id',activity_id,'type',activity_type,'title',title,'detail',detail,'questionId',question_id,'conceptId',concept_id,
   'route',route,'metadata',metadata,'at',occurred_at) order by occurred_at desc) from rows),'[]'::jsonb)) end;
$$;
...
begin
 ...
 select jsonb_build_object(
   ...
   'context_targeted',(select count(*) from english.learning_route_state where user_id=uid and route='targeted'
      and ('Learner Context'=any(coalesce(origins,'{}'::text[])) or 'Learner Context Related'=any(coalesce(origins,'{}'::text[])))),
   'recent_context',(select coalesce(jsonb_agg(to_jsonb(x) order by x.created_at desc),'[]'::jsonb) from(
      select note_id,question_id,left(note,160) note,processing_status,ai_status,diagnosis,created_at
      from english.learner_context_notes where user_id=uid order by created_at desc limit 6)x),
   'today_activity',public.english_get_learning_activity_today()) into outv;
 return outv;
end $$;
...
-- Keep the optimized Home snapshot shape, adding only a cheap Targeted summary.
...
saved as(select count(*) filter(where not mastered)::int eligible,count(*) filter(where not mastered and due)::int due
  from uid cross join lateral english.saved_revision_candidates(uid.id)),
starred as(select count(*) filter(where starred and not mastered)::int focus,count(*) filter(where difficult and starred and not mastered)::int difficult
  from uid cross join lateral english.starred_manual_index(uid.id)),
bank as(select count(*)::int total,count(*) filter(where coalesce(s.attempts,0)>0)::int exposed from uid
...
phrasal as(select count(*)::int today_count from uid join english.questions q on uid.id is not null and q.active
  and english.question_visible_to_user(uid.id,q.question_id) and q.source_id='PHRASAL_DAILY_'||to_char((now() at time zone 'Asia/Kolkata')::date,'YYYYMMDD')
  and (english.canonical_category(q.topic)='PHRASAL' or lower(btrim(coalesce(q.topic,'')))='phrasal verb')),
hindu as(select coalesce(jsonb_agg(jsonb_build_object('id',h.hindu_id) order by h.hindu_id),'[]'::jsonb) rows
  from uid join english.hindu_words h on uid.id is not null and h.active and h.word_date=(now() at time zone 'Asia/Kolkata')::date),
targeted as(select count(*)::int active,count(*) filter(where coalesce(ce.next_review,now())<=now())::int due_now
  from uid join english.learning_route_state r on r.user_id=uid.id and r.route='targeted'
  left join english.question_concept_mappings m on m.question_id=r.question_id left join english.concept_evidence ce on ce.user_id=uid.id and ce.concept_id=m.concept_id)
select case when uid.id is null then jsonb_build_object('ok',false,'error','Authentication required') else jsonb_build_object(
 'ok',true,'studyDay',greatest(1,((now() at time zone 'Asia/Kolkata')::date-date '2026-08-14')+1),'summary',summary.value,
 'intelligence',jsonb_build_object('daily',jsonb_build_object('actionableRemaining',coalesce((summary.value->>'daily_remaining')::int,0),
   'suppressed',coalesce((summary.value->>'daily_suppressed')::int,0)),'coreCoverage',jsonb_build_object('percent',case when bank.total>0 then round(bank.exposed*100.0/bank.total,1) else 0 end)),
...
-- Backfill legacy context into the new lifecycle. DISTINCT ON prevents duplicate conflict rows in one statement.
with source_rows as(
  select n.user_id,m.concept_id primary_concept_id,n.question_id primary_question_id,rm.concept_id related_concept_id,
    qx.rid related_question_id,term related_term,md5(lower(m.concept_id)||'|'||lower(term)) confusion_key,n.note_id,n.created_at,
    row_number() over(partition by n.user_id,md5(lower(m.concept_id)||'|'||lower(term)) order by n.created_at desc,n.note_id desc) rn
...
insert into english.learner_confusions(user_id,primary_concept_id,primary_question_id,related_concept_id,related_question_id,related_term,
  confusion_key,source_note_id,status,strength,opened_at,last_signal_at,metadata,updated_at)
select user_id,primary_concept_id,primary_question_id,related_concept_id,related_question_id,related_term,confusion_key,note_id,'open',1,
  created_at,created_at,jsonb_build_object('source','context','processor','backfill_v2'),now()
from source_rows where rn=1
...
select n.user_id,'context_targeted','Context added to Targeted',
  case when jsonb_array_length(coalesce(n.diagnosis->'related_terms','[]'::jsonb))>0 then 'Confusion detected · added to focused Targeted repair'
       else 'Learning context routed for focused repair' end,n.question_id,m.concept_id,n.note_id,'targeted',
  jsonb_build_object('diagnosis',n.diagnosis->>'type','backfill',true),n.created_at
...
-- Recompute only concepts touched by context/guess signals so unresolved-state semantics become live immediately.
do $$ declare r record; begin
  for r in select distinct user_id,concept_id from(
    select n.user_id,m.concept_id from english.learner_context_notes n join english.question_concept_mappings m on m.question_id=n.question_id
    union all
    select g.user_id,m.concept_id from english.learner_confidence_signals g join english.question_concept_mappings m on m.question_id=g.question_id
  )x where concept_id is not null loop
