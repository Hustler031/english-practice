...
begin
 ...
 with base as (
  select i.question_id,min(i.sequence)::int seq,q.*,coalesce(s.status,'New') st,coalesce(s.wrong,0) wr,coalesce(s.attempts,0) att,coalesce(s.last_marked,false) starred
  ...
  group by i.question_id,q.question_id,s.status,s.wrong,s.attempts,s.last_marked
 ), ranked as (
  select b.question_id,row_number() over(order by
    case when v_mode='random' and (b.st in ('Persistent Weak','Weak','Fragile') or b.wr>0) then 0 when v_mode='random' and english.recent_content_date(b)>=((now() at time zone 'Asia/Kolkata')::date-6) then 1 when v_mode='random' and b.starred then 2 when v_mode='random' and b.att>0 then 3 else 4 end,
    case when v_mode in ('weak','random') then random() else b.seq::double precision end,b.question_id)::int ord
  from base b
 ), chosen as (
  select * from ranked order by ord limit case when v_mode='all' then 100 else v_n end
 )
