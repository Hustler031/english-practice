...
-- Deterministic/bank-first remains the default. Luna is invoked only for context notes that need interpretation
-- or Targeted concepts that lack a fresh transfer item. No learner-facing AI response is produced.
...
begin
  ...
  -- Bank first: never generate when another active owner-visible item already proves this concept.
  if exists(
    select 1 from english.questions q join english.question_concept_mappings m on m.question_id=q.question_id
  ...
  insert into english.targeted_transfer_jobs(user_id,concept_id,source_question_id,source_note_id,related_term,reason,status,metadata)
  values(p_user_id,p_concept_id,p_source_question_id,p_source_note_id,nullif(trim(coalesce(p_related_term,'')),''),
    left(coalesce(nullif(trim(p_reason),''),'missing_transfer'),240),'queued',jsonb_build_object('bank_first_checked_at',now()))
  on conflict(user_id,concept_id,source_question_id) do update set
    ...
    status=case when english.targeted_transfer_jobs.status='failed' and english.targeted_transfer_jobs.attempts<3 then 'queued'
      else english.targeted_transfer_jobs.status end,
    next_attempt_at=case when english.targeted_transfer_jobs.status='failed' and english.targeted_transfer_jobs.attempts<3 then now()
      else english.targeted_transfer_jobs.next_attempt_at end,
    updated_at=now()
...
create or replace function english.enqueue_missing_targeted_transfers(p_limit integer default 20)
...
begin
  for r in
    select lr.user_id,lr.question_id,m.concept_id,
      nullif(lr.metadata->>'source_note_id','')::uuid source_note_id,
      case when coalesce(lr.metadata->>'targeted_kind','')='confusion' then 'Targeted confusion needs fresh transfer'
        when coalesce(lr.metadata->>'targeted_kind','')='transfer_check' then 'I Guessed / transfer validation lacks alternate item'
        else 'Targeted concept lacks alternate transfer item' end reason
    from english.learning_route_state lr join english.question_concept_mappings m on m.question_id=lr.question_id
    where lr.route='targeted' and coalesce(lr.metadata->>'targeted_kind','') in ('confusion','transfer_check','need_learning')
      and not exists(
    ...
    order by case coalesce(lr.metadata->>'targeted_kind','') when 'confusion' then 1 when 'transfer_check' then 2 else 3 end,lr.updated_at
...
create or replace function english.context_claim(p_token text,p_limit integer default 8)
...
begin
  ...
  update english.learner_context_notes set ai_status='queued',ai_next_attempt_at=now(),ai_error='stale background processing recovered'
  where ai_status='processing' and ai_attempted_at<now()-interval '5 minutes' and ai_attempts<3;

  with pick as(
    select n.note_id from english.learner_context_notes n
    ...
    order by n.created_at,n.note_id for update skip locked limit greatest(1,least(12,coalesce(p_limit,8)))
  ), upd as(
    update english.learner_context_notes n set ai_status='processing',ai_attempts=n.ai_attempts+1,ai_attempted_at=now(),ai_error=null
    from pick p where n.note_id=p.note_id returning n.*
  ), payload as(
    select u.note_id,u.user_id,u.question_id,u.note,u.context_snapshot,u.diagnosis,
      ...
      (select count(*) from english.questions q2 join english.question_concept_mappings m2 on m2.question_id=q2.question_id
    ...
    from upd u left join english.question_concept_mappings m on m.question_id=u.question_id
    left join english.concepts c on c.concept_id=m.concept_id left join english.concept_evidence ce on ce.user_id=u.user_id and ce.concept_id=m.concept_id
...
create or replace function public.english_context_claim(p_token text,p_limit integer default 8)
...
as $$
declare
  n english.learner_context_notes%rowtype; cid text; dtype text; action text; kind text; requires_transfer boolean;
  terms text[]:='{}'::text[]; term text; rid text; rcid text; matched int:=0; job uuid; detail text;
begin
  if not english.context_worker_authorized(p_token) then raise exception 'context worker unauthorized'; end if;
  ...
  if n.ai_status='done' then return jsonb_build_object('ok',true,'already_done',true); end if;
  ...
  select coalesce(array_agg(value),'{}'::text[]) into terms from jsonb_array_elements_text(coalesce(p_diagnosis->'relatedTerms','[]'::jsonb));
  kind:=case when dtype in('confusion_pair','lexical_interference') then 'confusion'
    ...
    when dtype='no_action' then null else 'need_learning' end;
  ...
  if cid is not null and dtype<>'no_action' then
    perform english.route_to_targeted(n.user_id,n.question_id,'Background Context AI',
      case kind when 'confusion' then 'Background analysis confirmed a learner confusion'
        ...
        else 'Background analysis confirmed a focused learning gap' end);
  ...
  foreach term in array terms loop
    term:=nullif(trim(term),''); if term is null then continue; end if;
    select q.question_id,m.concept_id into rid,rcid
    from english.questions q left join english.question_concept_mappings m on m.question_id=q.question_id
    ...
    order by case when lower(trim(coalesce(q.word,'')))=lower(term) then 0 else 1 end,q.question_id limit 1;
  ...
  if cid is not null and (requires_transfer or kind='transfer_check') then
    job:=english.ensure_transfer_generation_job(n.user_id,cid,n.question_id,p_note_id,case when cardinality(terms)>0 then terms[1] else null end,
      'Background context analysis requested fresh transfer');
  end if;

  update english.learner_context_notes set ai_status='done',ai_error=null,ai_next_attempt_at=null,processed_at=coalesce(processed_at,now()),
  ...
  detail:=case when dtype='no_action' then 'Background analysis found no extra learning action needed.'
    when kind='confusion' then 'Confusion interpreted'||case when matched>0 then ' · existing bank items added to Targeted' else ' · focused Targeted repair created' end
    when kind='retention_check' then 'Retention issue interpreted · spaced validation added to Targeted'
    when kind='transfer_check' then 'Fresh transfer evidence requested'||case when job is not null then ' · generation queued' else ' · existing bank evidence available' end
    else 'Learning gap interpreted · added to Targeted' end;
  perform english.log_learning_activity(n.user_id,case when dtype='no_action' then 'context_ai_no_action' else 'context_ai_targeted' end,
    case when dtype='no_action' then 'Context analysed' else 'Background analysis → Targeted' end,detail,n.question_id,cid,p_note_id,
    case when dtype='no_action' then null else 'targeted' end,jsonb_build_object('diagnosis',dtype,'model',p_model,'transferJob',job),now());
...
create or replace function public.english_apply_context_ai_diagnosis(p_token text,p_note_id uuid,p_diagnosis jsonb,p_model text default 'gpt-5.6-luna',p_usage jsonb default '{}'::jsonb)
returns jsonb language sql security definer set search_path to 'pg_catalog','public','english'
...
begin
  ...
  update english.learner_context_notes set ai_status=case when coalesce(a,0)>=3 then 'failed' else 'queued' end,
    ai_next_attempt_at=case when coalesce(a,0)>=3 then null else now()+make_interval(mins=>least(30,coalesce(a,1)*5)) end,
    ai_error=left(coalesce(p_error,'background analysis failed'),1000) where note_id=p_note_id;
end $$;
...
create or replace function english.transfer_claim(p_token text,p_limit integer default 2)
returns jsonb language plpgsql security definer set search_path to 'pg_catalog','english'
as $$
declare outv jsonb;
begin
  ...
  with pick as(
    select job_id from english.targeted_transfer_jobs where status='queued' and attempts<3 and (next_attempt_at is null or next_attempt_at<=now())
    order by created_at,job_id for update skip locked limit greatest(1,least(4,coalesce(p_limit,2)))
  ), upd as(
...
create or replace function public.english_transfer_claim(p_token text,p_limit integer default 2)
returns jsonb language sql security definer set search_path to 'pg_catalog','public','english'
...
as $$
declare
  j english.targeted_transfer_jobs%rowtype; src english.questions%rowtype; qid text;
  ...
  quality numeric:=coalesce((p_item->>'qualityScore')::numeric,0); ambiguous boolean:=coalesce((p_item->>'ambiguous')::boolean,true);
  fam text;
begin
  if not english.context_worker_authorized(p_token) then raise exception 'context worker unauthorized'; end if;
  ...
  if j.status='done' then return jsonb_build_object('ok',true,'already_done',true,'questionId',j.generated_question_id); end if;
  ...
  select family_id into fam from english.question_concept_mappings where question_id=j.source_question_id;
  qid:='AIT_'||upper(substr(replace(j.job_id::text,'-',''),1,16));
  insert into english.questions(question_id,topic,word,question,option_a,option_b,option_c,option_d,correct,explanation,subtopic,question_type,
    source_file,source_page,concept_id,difficulty,source_id,learning_status,content_status,exam_relevance,tip,usage_note,example_sentence,memory_aid,related_words,review_notes,active)
  values(qid,coalesce(src.topic,j.skill_family),nullif(trim(coalesce(p_item->>'word','')),''),qtext,qa,qb,qc,qd,ckey,
    trim(coalesce(p_item->>'explanation','Fresh transfer validation.')),src.subtopic,coalesce(nullif(trim(p_item->>'questionType'),''),'Transfer / Discrimination'),
    'AI Targeted Transfer',null,j.concept_id,coalesce(nullif(trim(p_item->>'difficulty'),''),'Moderate'),'AI_TARGETED_'||upper(substr(replace(j.job_id::text,'-',''),1,12)),
    'New','Active',coalesce(src.exam_relevance,'high'),null,null,null,null,j.related_term,
  ...
  insert into english.question_origins(question_id,origin_kind,origin_ref,owner_user_id)
    values(qid,'targeted_generated',j.job_id::text,j.user_id) on conflict(question_id) do update set origin_kind=excluded.origin_kind,origin_ref=excluded.origin_ref,owner_user_id=excluded.owner_user_id;
  insert into english.question_concept_mappings(question_id,concept_id,family_id,mapping_confidence,mapping_method,model,review_status,relation_type,updated_at)
    values(qid,j.concept_id,fam,1,'luna_validated_transfer',p_model,'verified','transfer',now())
    on conflict(question_id) do update set concept_id=excluded.concept_id,family_id=excluded.family_id,mapping_confidence=1,
      mapping_method=excluded.mapping_method,model=excluded.model,review_status='verified',relation_type='transfer',updated_at=now();
  ...
  update english.targeted_transfer_jobs set status='done',generated_question_id=qid,processed_at=now(),updated_at=now(),last_error=null,
    metadata=metadata||jsonb_build_object('model',p_model,'qualityScore',quality,'usage',coalesce(p_usage,'{}'::jsonb)) where job_id=j.job_id;
  perform english.log_learning_activity(j.user_id,'transfer_generated','Fresh transfer prepared',
    'Existing bank had no alternate item · a validated transfer question was added to Targeted.',qid,j.concept_id,j.source_note_id,'targeted',
...
begin
  ...
  update english.targeted_transfer_jobs set status=case when coalesce(a,0)>=3 then 'failed' else 'queued' end,
    next_attempt_at=case when coalesce(a,0)>=3 then null else now()+make_interval(mins=>least(30,coalesce(a,1)*5)) end,
    last_error=left(coalesce(p_error,'transfer generation failed'),1000),updated_at=now() where job_id=p_job_id;
end $$;
...
-- Owner-only visibility for generated targeted transfer questions.
...
as $$
select case when p_user_id is null then false when o.question_id is null then true
  when o.origin_kind in ('saved_generated','targeted_generated') then o.owner_user_id=p_user_id else true end
from(select 1)x left join english.question_origins o on o.question_id=p_question_id;
$$;

-- Re-run I Guessed with transfer generation only when the bank lacks an alternate.
...
as $$
declare uid uuid:=auth.uid(); cid text; aid text:=nullif(trim(coalesce(p_attempt_id,'')),''); inserted boolean:=false; alt_exists boolean:=false; job uuid;
begin
  ...
  if aid is not null then insert into english.learner_confidence_signals(user_id,question_id,attempt_id,signal) values(uid,p_question_id,aid,'guessed') on conflict(user_id,question_id,attempt_id,signal) where attempt_id is not null do nothing; get diagnostics inserted=row_count;
  elsif not exists(select 1 from english.learner_confidence_signals where user_id=uid and question_id=p_question_id and signal='guessed' and attempt_id is null and created_at>now()-interval '10 minutes') then insert into english.learner_confidence_signals(user_id,question_id,attempt_id,signal) values(uid,p_question_id,null,'guessed'); inserted:=true; end if;
  ...
  if inserted then
    ...
    update english.learning_route_state set metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('targeted_kind','transfer_check','source_attempt_id',aid),updated_at=now() where user_id=uid and question_id=p_question_id;
    if cid is not null and not alt_exists then job:=english.ensure_transfer_generation_job(uid,cid,p_question_id,null,null,'I Guessed requires a fresh transfer but bank has no alternate'); end if;
    perform english.log_learning_activity(uid,'guess_transfer','I Guessed → transfer check',case when job is null then 'Correctness is preserved; fresh validation is queued.' else 'Correctness is preserved; a missing transfer item is being prepared.' end,p_question_id,cid,null,'targeted',jsonb_build_object('attempt_id',aid,'transferJob',job),now());
  end if;
  if cid is not null and inserted then perform english.recompute_concept_evidence(uid,cid); end if;
  return jsonb_build_object('ok',true,'signal','guessed','concept_id',cid,'attempt_id',aid,'recorded',inserted,'validation_due',case when inserted then now()+interval '12 hours' else null end,'alternate_available',alt_exists,'transfer_job',job);
end $$;

create or replace function english.kick_context_worker(p_context_limit integer default 8,p_transfer_limit integer default 2)
...
begin
  perform english.enqueue_missing_targeted_transfers(20);
  update english.learner_context_notes set ai_status='queued',ai_next_attempt_at=now(),ai_error='stale background processing recovered'
    where ai_status='processing' and ai_attempted_at<now()-interval '5 minutes' and ai_attempts<3;
  update english.targeted_transfer_jobs set status='queued',next_attempt_at=now(),last_error='stale generation recovered',updated_at=now()
    where status='processing' and updated_at<now()-interval '5 minutes' and attempts<3;
  ...
  select net.http_post(
    ...
    body:=jsonb_build_object('contextLimit',greatest(1,least(12,coalesce(p_context_limit,8))),'transferLimit',greatest(1,least(4,coalesce(p_transfer_limit,2)))),
    params:='{}'::jsonb,headers:=jsonb_build_object('Content-Type','application/json','x-english-context-token',v_token),timeout_milliseconds:=55000
  ) into v_id;
  return v_id;
end $$;
...
-- Replace only this named cron job; unrelated schedules remain untouched.
do $$ declare j record; begin
  for j in select jobid from cron.job where command ilike '%kick_context_worker%' loop perform cron.unschedule(j.jobid); end loop;
  perform cron.schedule('english-context-intelligence','*/2 * * * *','select english.kick_context_worker(8,2);');
end $$;
