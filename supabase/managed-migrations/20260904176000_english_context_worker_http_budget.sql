-- The worker may perform a transfer/revision generator + critic sequentially.
-- Each AI call has an 18s safety timeout, so the scheduler's prior 28s pg_net
-- timeout could abort a healthy invocation before the worker returned.
-- Restore the proven 55s caller budget; item-level AI timeouts remain bounded.
