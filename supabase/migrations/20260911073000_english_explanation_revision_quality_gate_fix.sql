-- Explanation-only repairs must be judged on the explanation repair itself.
-- Pre-existing weak distractors are immutable in this lane and must not depress an otherwise
-- accurate explanation below the whole-item qualityScore threshold.
...
begin
  ...
  -- Universal safety checks still apply. The whole-item score threshold applies only when
  -- the lane is allowed to repair the whole item. explanation_weak is explanation-only.
