...
begin
  ...
  with focus as(
    ...
    select r.question_id,m.concept_id,r.metadata,r.origins,r.last_route_reason,r.updated_at,ce.coverage_state,ce.confidence_score,ce.next_review,
      ...
      (lower(coalesce(r.metadata->>'userCurated','false'))='true') user_curated,
      coalesce(qm.too_easy,false) question_too_easy,coalesce(qm.observed_difficulty,0.5) question_observed_difficulty
    ...
    select f.*,coalesce(alt.question_id,f.question_id) delivery_question_id,row_number() over(partition by coalesce(f.concept_id,f.question_id)
      order by case when f.kind='confusion' then 1 when f.user_curated then 2 when f.kind='transfer_check' then 3 when f.kind='retention_check' then 4 else 5 end,
      f.updated_at desc) concept_pick
    ...
    select * from delivery where concept_pick=1 order by
      case when kind='confusion' then 1 when user_curated then 2 when kind='transfer_check' then 3 when kind='retention_check' and (next_review is null or next_review<=now()) then 4 when kind='need_learning' then 5 else 6 end,
  ...
  select coalesce(jsonb_agg(english.question_payload(uid,c.delivery_question_id)||jsonb_build_object(
    ...
    'observedDifficulty',c.question_observed_difficulty,'questionTooEasy',c.question_too_easy,'userCurated',c.user_curated
  ) order by case when c.kind='confusion' then 1 when c.user_curated then 2 when c.kind='transfer_check' then 3 when c.kind='retention_check' and (c.next_review is null or c.next_review<=now()) then 4 when c.kind='need_learning' then 5 else 6 end,
...
create or replace function public.english_start_targeted_fresh_session(p_count integer default 15, p_kind text default null::text, p_confusion_id uuid default null::uuid, p_session_nonce text default null::text, p_client_exclude text[] default '{}'::text[], p_due_only boolean default false)
returns jsonb
language plpgsql
security definer
set search_path to 'pg_catalog','public','english','auth'
...
begin
  ...
  if coalesce(p_due_only,false) then
    ...
    where e.item->>'targetedKind'='confusion'
       or lower(coalesce(e.item->>'userCurated','false'))='true'
