-- Review Due must not independently schedule concepts currently owned by Fast Track.
-- Ownership covers ready, waiting and retention_watch. If a concept later leaves
-- Fast Track (for example after failure -> Targeted), normal Review Due ownership
-- resumes automatically.

create or replace function english.review_due_fast_track_owned(
  p_user_id uuid,
  p_concept_key text
)
returns boolean
...
select exists (
  ...
  where r.user_id=p_user_id
    ...
    and coalesce(r.fast_track_status,'ready') in ('ready','waiting','retention_watch')
    and english.review_due_concept_key(r.question_id)=p_concept_key
);
$function$;
...
begin
  ...
  if found then
    ...
    where o.user_id=p_user_id and o.due_date=p_due_date
      ...
      and not english.review_due_fast_track_owned(p_user_id,o.concept_key);

    return jsonb_build_object(
      'ok',true,'unchanged',true,'date',p_due_date,
      'dueQuestions',(
        select coalesce(sum(o.due_question_count),0)::int
        from english.review_due_obligations o
        where o.user_id=p_user_id and o.due_date=p_due_date
          and not english.review_due_fast_track_owned(p_user_id,o.concept_key)
      ),
  ...
  with exact_grouped as (
    ...
    where s.user_id=p_user_id
      ...
      and not english.review_due_fast_track_owned(
        p_user_id,
        english.review_due_concept_key(s.question_id)
      )
  ...
  insert into english.review_due_day_runs(
    ...
    p_user_id,p_due_date,now(),v_questions,v_concepts,3
  );
...
create or replace function public.english_get_review_due_lane(p_nonce text default null::text)
returns jsonb
language sql
stable
security definer
...
with params as (
  ...
  where not english.review_due_fast_track_owned(o.user_id,o.concept_key)
  ...
  cross join lateral (
    ...
    where q0.concept_id=s.concept_key
      and not exists(
        select 1 from english.question_concept_mappings mx where mx.question_id=q0.question_id
      )
    union
    select q0.question_id
    from english.questions q0
...
with params as (
  ...
  select
    coalesce(sum(o.due_question_count),0)::integer due_question_count,
    count(*)::integer due_concept_count
  from obligations o
)
select case
  when (select uid from params) is null then jsonb_build_object('ok',false,'reason','Authentication required')
    ...
    'dueQuestionCount',coalesce((select due_question_count from effective_run),0),
