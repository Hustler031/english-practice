-- My Saved: event-driven enrichment after a successful Add Word capture.
-- The worker wake-up is asynchronous through pg_net and deliberately limited
-- to one item. The existing hourly cron remains the recovery/safety net.
...
begin
  ...
  if v_word = '' then
    raise exception 'Enter a word first.';
  end if;
  if v_capture not in ('AUTO','V','SM','OWS','PV','IP') then
  ...
  where user_id = uid
    and active
    and lower(btrim(coalesce(word,''))) = lower(v_word)
  ...
  if found then
    ...
    set context = case when btrim(coalesce(p_context,'')) <> '' then btrim(p_context) else context end,
        ...
        updated_at = now(),
        gpt_status = case
          when coalesce(btrim(gpt_status),'') = '' and coalesce(btrim(practice_question_id),'') = '' then 'Pending GPT'
          else gpt_status
        end
    where saved_id = s.saved_id
    returning * into s;

    v_resolved := english.resolve_saved_type(
      v_capture,s.word,s.meaning,s.context,s.part_of_speech,s.question,s.explanation
    );
    ...
    -- Only wake the worker when this duplicate is actually still pending.
    -- Failure to enqueue enrichment must never make the user's save fail;
    -- the hourly cron remains the recovery path.
    if lower(btrim(coalesce(s.gpt_status,''))) in ('', 'pending gpt', 'needs enrichment') then
      ...
      exception when others then
        raise warning 'My Saved immediate enrichment kick failed for %: %', s.saved_id, sqlerrm;
      end;
    end if;
    ...
    return jsonb_build_object(
      ...
      'gpt_status',coalesce(s.gpt_status,'Pending GPT'),
      'capture_type',v_capture,
  ...
  insert into english.saved_items(
    ...
    created_at,updated_at,status,practice_question_id,active,part_of_speech,
    synonyms,antonyms,example,explanation,question,option_a,option_b,option_c,
  ...
  -- pg_net sends only after the surrounding transaction commits, so the worker
  -- can see both the saved item and its capture type. limit=1 keeps generation
  -- one-item-at-a-time and the worker lease prevents duplicate active work.
  begin
    perform english.kick_saved_enrichment_worker(1);
  exception when others then
...
comment on function public.english_save_word(text,text,text,text,text,text) is
  'Saves a My Saved capture and asynchronously wakes the one-item enrichment worker; hourly cron remains fallback.';
