-- English V2 behavior-parity tables.
-- Production already received the equivalent migration through the managed Supabase migration API.
-- Keep this file as the repository source for future/reproducible environments.
...
create index if not exists english_star_events_current_idx
  on english.star_events(user_id, question_id, event_at desc);
...
create index if not exists english_mastery_events_current_idx
  on english.mastery_events(user_id, question_id, mastered_on desc, id desc);
...
create index if not exists english_daily_current_date_idx
  on english.daily_current(user_id, quiz_date, sequence);
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='saved_item_types' and policyname='english_saved_item_types_own') then
  ...
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='star_events' and policyname='english_star_events_own') then
  ...
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='difficult_state' and policyname='english_difficult_state_own') then
  ...
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='mastery_events' and policyname='english_mastery_events_own') then
  ...
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='daily_current' and policyname='english_daily_current_own') then
  ...
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='hindu_words' and policyname='english_hindu_words_authenticated_read') then
    create policy english_hindu_words_authenticated_read on english.hindu_words for select to authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname='english' and tablename='recall_checks' and policyname='english_recall_checks_authenticated_read') then
