-- REVIEW DUE TODAY — EXACT-DUE QUALITY SEMANTICS
-- A correct answer to the exact scheduled due question advances that question's own
-- scheduler through the normal submit path, so `too_easy` must not make the due
-- obligation impossible to close. `too_easy` continues to disqualify sibling/cross-credit
-- evidence. Guessed evidence remains low-confidence and needs a fresh/aged confirmation.
...
select
  ...
  case
    ...
    when wrong_at is not null then 'needs_repair'
    when low_at is not null
      and (
        coalesce((select low_q=any(o.due_question_ids) from obligation o),false)
        or cross_credit_enabled
      ) then 'low_confidence'
    else 'remaining'
