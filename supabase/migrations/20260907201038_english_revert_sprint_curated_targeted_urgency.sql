...
declare
  ddl text;
begin
  ddl:=pg_get_functiondef('english.promote_sprint_bank_item(uuid,uuid,integer)'::regprocedure);
  ddl:=replace(
    ddl,
    E'\n  perform english.route_sprint_bank_to_targeted(p_uid,qid,p_session_id,p_position);\n',
    E'\n'
  );
  if position('route_sprint_bank_to_targeted' in ddl)>0 then
    raise exception 'Could not remove Sprint Bank Targeted routing hook';
  end if;
  execute ddl;
end
$restore_promote$;
...
create or replace function public.english_get_targeted_batch(
  ...
  p_confusion_id uuid default null::uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  with focus as(
    select *
    from english.learner_confusions
    where user_id=uid
      and p_confusion_id is not null
      and confusion_id=p_confusion_id
  ), routed as(
    select
      r.question_id,m.concept_id,r.metadata,r.origins,r.last_route_reason,r.updated_at,
      ce.coverage_state,ce.confidence_score,ce.next_review,
      english.targeted_route_kind(r.metadata,r.origins) kind,
      coalesce(qm.too_easy,false) question_too_easy,
      coalesce(qm.observed_difficulty,0.5) question_observed_difficulty
    ...
    from routed x
    where (
      p_confusion_id is null
      or exists(
        select 1 from focus f
        where x.concept_id=f.primary_concept_id
           or x.concept_id=f.related_concept_id
           or x.question_id=f.primary_question_id
           or x.question_id=f.related_question_id
      )
    )
    ...
    select
      f.*,
      coalesce(alt.question_id,f.question_id) delivery_question_id,
      row_number() over(
        partition by coalesce(f.concept_id,f.question_id)
        order by case f.kind
          when 'confusion' then 1
          when 'transfer_check' then 2
          when 'retention_check' then 3
          else 4
        end,
        f.updated_at desc
      ) concept_pick
    ...
    left join lateral(
      ...
      where f.kind in('transfer_check','confusion')
        and not (
          f.kind='transfer_check'
          and (
            'AI Transfer'=any(coalesce(f.origins,'{}'::text[]))
            or exists(
              select 1
              from english.question_origins qo
              where qo.question_id=f.question_id
                and qo.origin_kind='targeted_generated'
                and qo.owner_user_id=uid
            )
          )
        )
        and f.concept_id is not null
        and m2.concept_id=f.concept_id
        and q2.question_id<>f.question_id
        and q2.active
    ...
    order by
      ...
      case kind
        ...
        when 'transfer_check' then 2
        when 'retention_check' then 3
        when 'need_learning' then 4
        else 5
      end,
      case when kind='need_learning' and question_too_easy then 1 else 0 end,
      question_observed_difficulty desc,
      case when session_nonce is not null then md5(session_nonce||'|'||coalesce(concept_id,question_id)) else '' end,
      coalesce(next_review,'epoch'::timestamptz),
      updated_at desc
    limit n
  )
  select coalesce(
    jsonb_agg(
      english.question_payload(uid,c.delivery_question_id)
      || jsonb_build_object(
        'learningRoute','targeted',
        'targetedKind',c.kind,
        'targetedReason',c.last_route_reason,
        'sourceQuestionId',c.question_id,
        'conceptId',c.concept_id,
        'conceptCoverage',coalesce(c.coverage_state,'unseen'),
        'conceptConfidence',coalesce(c.confidence_score,0),
        'conceptNextReview',c.next_review,
        'observedDifficulty',c.question_observed_difficulty,
        'questionTooEasy',c.question_too_easy
      )
      order by
        case
        ...
        case when c.kind='need_learning' and c.question_too_easy then 1 else 0 end,
        c.question_observed_difficulty desc,
        c.updated_at desc
    ),
    '[]'::jsonb
  ) into outv
...
create or replace function public.english_start_targeted_fresh_session(
  ...
  p_kind text default null::text,
  p_confusion_id uuid default null::uuid,
  p_session_nonce text default null::text,
...
where r.route='targeted'
  and r.origins=array['Sprint Bank']::text[]
  and r.last_route_reason='User Curated · Saved from Sprint'
  and lower(coalesce(r.metadata->>'userCurated','false'))='true'
  and coalesce(r.metadata->>'userCuratedSource','')='sprint_bank';

drop function if exists english.route_sprint_bank_to_targeted(uuid,text,uuid,integer);
