-- Saved enrichment must remain scheduler-paced. A successful item must not immediately
-- self-kick the next item, because chained success bursts can consume provider quota
-- independently of the configured cron cadence.
...
begin
  ...
  -- Intentionally no success self-kick. Cron/recovery cadence owns pacing.
