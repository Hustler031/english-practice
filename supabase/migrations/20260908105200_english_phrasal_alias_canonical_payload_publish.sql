-- Follow-up to alias-safe Phrasal publish: duplicate reconciliation may retain a
-- different canonical explanation while question/options/key remain identical.
-- For exact-front aliases, publish the active canonical Question_ID and its current
-- canonical explanation. The frozen selection, concept, question, options and key
...
begin
  ...
  select coalesce(jsonb_agg(
    case
      ...
      then coalesce((
        select jsonb_set(
                 jsonb_set(e.value,'{baseQuestionId}',to_jsonb(a.canonical_question_id),true),
                 '{explanation}',to_jsonb(coalesce(q.explanation,'')),true
               )
