-- Source Library is an archive/practice surface, not the 7/14-day New Practice window.
-- Keep existing user-state exclusions but allow every dated source batch to be browsed.
...
with pool as (
 ...
 where auth.uid() is not null and q.active and english.question_visible_to_user(auth.uid(),q.question_id)
   and not coalesce(s.mastered,false) and english.source_descriptor_key(q) is not null
), g as (
 select source_key,min(source_name) name,count(*) count,count(*) filter(where st in ('Persistent Weak','Weak','Fragile')) weak,
        count(*) filter(where added>=((now() at time zone 'Asia/Kolkata')::date-6)) recent,max(added) latest,
        string_agg(distinct english.canonical_category(topic),', ' order by english.canonical_category(topic)) categories
 from pool group by source_key
)
select coalesce(jsonb_agg(jsonb_build_object(
 'key',g.source_key,'name',g.name,'count',g.count,'weak',g.weak,'recent',g.recent,'latest',g.latest,'categorySummary',g.categories,
 'children',case when g.source_key='THE_HINDU' then (
   select coalesce(jsonb_agg(jsonb_build_object('key','THE_HINDU::'||x.added::text,'date',x.added,'count',x.n,'weak',x.w) order by x.added desc),'[]'::jsonb)
   from (select p.added,count(*) n,count(*) filter(where p.st in ('Persistent Weak','Weak','Fragile')) w from pool p where p.source_key='THE_HINDU' and p.added is not null group by p.added) x
 ) else '[]'::jsonb end
) order by case g.source_key when 'HANDWRITTEN' then 1 when 'THE_HINDU' then 2 when 'SCREENSHOTS' then 3 else 4 end,g.latest desc nulls last,g.name),'[]'::jsonb) from g;
$function$;
