...
[
  ...
  ['Performance Attempt_ID column missing','batch fails closed if dedupe column is unavailable'],
...
[
  ...
  ['if(!String(p.module||\'\').trim())p.module=','legacy V3 records missing module are normalized at send time'],
  ...
  ['ready=Number(x.nextAt||0)<=now','backed-off item does not block other ready attempts'],
  ['blocked.has(id)','a backed-off earlier attempt blocks only newer attempts for the same Question_ID'],
