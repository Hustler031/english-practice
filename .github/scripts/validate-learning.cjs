...
const server=['LearningIntelligence.gs','BankCoverage.gs','DailyAdaptive.gs','LearningProgress.gs','MySavedRevision.gs','LearningHinduCompat.gs'];
...
['learningBankEligibleQuestionsV2_','facts.seen.has(id)','Math.min(10-doneToday,unseen)','q.difficult=!!diff[q.id]','q.marked=!!stars[q.id]'].forEach(x=>need(bank,x,`Bank Coverage contract ${x}`));
...
['function getHinduPracticeProgressCentral(','module===\'hindu\''].forEach(x=>need(hcompat,x,`Hindu central progress ${x}`));
...
[['function submitAnswerV2(',1],['function setCentralDifficult(',1],['function getLearningProgressSnapshot(',1],['function getBankCoverageHub(',1],['function getMySavedRevisionHub(',1]].forEach(([fn,n])=>count(allNew,fn)===n?ok(`Unique learning function: ${fn}`):fail(`Expected ${n} occurrence of ${fn}, found ${count(allNew,fn)}`));
