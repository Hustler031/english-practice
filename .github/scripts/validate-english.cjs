...
const required=['Code.gs','DailyV2.gs','Demand.gs','NewPractice.gs','SourcePractice.gs','HinduVocab.gs','DashboardStats.gs','ProgressStats.gs','Saved.gs','Index.html','Styles.html','AppJS.html','QuizJS.html','HinduUI.html','SourceUI.html','DemandUI.html','DashboardUI.html','SavedUI.html','appsscript.json'];
required.forEach(f=>fs.existsSync(path.join(root,f))?ok(`File present: ${f}`):fail(`Required Apps Script file missing: ${f}`));
if(process.exitCode)process.exit(process.exitCode);
const server=['Code.gs','DailyV2.gs','Demand.gs','NewPractice.gs','SourcePractice.gs','HinduVocab.gs','DashboardStats.gs','ProgressStats.gs','Saved.gs'];
const front=['AppJS.html','QuizJS.html','HinduUI.html','SourceUI.html','DemandUI.html','DashboardUI.html','SavedUI.html'];
...
const code=read('Code.gs'),daily=read('DailyV2.gs'),demand=read('Demand.gs'),np=read('NewPractice.gs'),source=read('SourcePractice.gs'),hindu=read('HinduVocab.gs'),dash=read('DashboardStats.gs'),progress=read('ProgressStats.gs'),saved=read('Saved.gs'),index=read('Index.html'),app=read('AppJS.html'),quiz=read('QuizJS.html'),dui=read('DemandUI.html'),sui=read('SourceUI.html'),hui=read('HinduUI.html'),dbui=read('DashboardUI.html'),savedui=read('SavedUI.html');
...
['function getNewPracticeHub(','function getNewPracticeBatch(','NOT_SPECIFIED','MY_SAVED_WORDS'].forEach(x=>need(np,x,x));
...
['function captureMyWord(','function updateMyWord(','function getMySavedHub(','function getStarredPracticeBatch(','function promoteMyWordToPractice(','My_Words'].forEach(x=>need(saved,x,x));
['id="homeView"','id="dailyStartBtn"','id="todayDoneBadge"',"include('DashboardUI')","include('SavedUI')",'Loading daily plan…','id="quizView"','onclick="EPQuiz.next()"','onclick="EPApp.openSaved()"','onclick="EPSaved.openCapture()"'].forEach(x=>need(index,x,x));
...
['Starred Questions','My Words','[10,20,50,100]','Add to Practice','openCapture'].forEach(x=>need(savedui,x,x));
