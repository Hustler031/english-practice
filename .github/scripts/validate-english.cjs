...
const required=['Code.gs','DailyV2.gs','Demand.gs','NewPractice.gs','NewPracticeLive.gs','SourcePractice.gs','TopicPractice.gs','HinduVocab.gs','DashboardStats.gs','ProgressStats.gs','Saved.gs','StarredRevision.gs','Index.html','Styles.html','AppJS.html','QuizJS.html','FastUI.html','TopicUI.html','NewPracticeUI.html','HinduUI.html','SourceUI.html','DemandUI.html','DashboardUI.html','SavedUI.html','MyWordsFinalUI.html','StarredRevisionUI.html','appsscript.json'];
required.forEach(f=>fs.existsSync(path.join(root,f))?ok(`File present: ${f}`):fail(`Required Apps Script file missing: ${f}`));
if(process.exitCode)process.exit(process.exitCode);
const server=['Code.gs','DailyV2.gs','Demand.gs','NewPractice.gs','NewPracticeLive.gs','SourcePractice.gs','TopicPractice.gs','HinduVocab.gs','DashboardStats.gs','ProgressStats.gs','Saved.gs','StarredRevision.gs'];
const front=['AppJS.html','QuizJS.html','FastUI.html','TopicUI.html','NewPracticeUI.html','HinduUI.html','SourceUI.html','DemandUI.html','DashboardUI.html','SavedUI.html','MyWordsFinalUI.html','StarredRevisionUI.html'];
...
const code=read('Code.gs'),daily=read('DailyV2.gs'),demand=read('Demand.gs'),np=read('NewPractice.gs'),npl=read('NewPracticeLive.gs'),source=read('SourcePractice.gs'),topic=read('TopicPractice.gs'),hindu=read('HinduVocab.gs'),dash=read('DashboardStats.gs'),progress=read('ProgressStats.gs'),saved=read('Saved.gs'),starred=read('StarredRevision.gs'),index=read('Index.html'),app=read('AppJS.html'),quiz=read('QuizJS.html'),fast=read('FastUI.html'),tui=read('TopicUI.html'),nui=read('NewPracticeUI.html'),dui=read('DemandUI.html'),sui=read('SourceUI.html'),hui=read('HinduUI.html'),dbui=read('DashboardUI.html'),savedui=read('SavedUI.html'),mwui=read('MyWordsFinalUI.html'),srui=read('StarredRevisionUI.html');
