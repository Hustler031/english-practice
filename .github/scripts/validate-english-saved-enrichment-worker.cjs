...
const workflow = fs.readFileSync(
  path.join(root, '.github/workflows/saved-enrichment-chatgpt-bridge.yml'),
  'utf8',
);
...
// The paid worker remains only as an explicit manual fallback. Its implementation is
// still required to stay bounded and helper-only in case it is invoked manually.
...
// Production recurring ownership is ChatGPT task -> GitHub queue -> OIDC bridge.
...
need(bridge, 'Hustler031/english-practice', 'repository claim pin');
need(bridge, 'refs/heads/automation/saved-enrichment', 'queue ref claim pin');
...
need(workflow, 'id-token: write', 'GitHub OIDC permission');
need(workflow, 'automation/saved-enrichment', 'dedicated queue branch');
need(workflow, 'audience=english-saved-enrichment', 'OIDC audience request');
need(workflow, 'english-saved-enrichment-bridge', 'Supabase bridge endpoint');
need(workflow, 'git push --force origin HEAD:automation/saved-enrichment', 'ephemeral queue ref reset');
need(workflow, '[bridge-sync]', 'internal push loop guard');
forbid(workflow, 'SUPABASE_SERVICE_ROLE_KEY', 'stored Supabase service-role secret');
forbid(workflow, 'OPENAI_API_KEY', 'OpenAI API secret');

console.log('English Saved enrichment ChatGPT bridge contract: PASS');
