...
[
  [quiz,"function isRecallCard(q)",'Existing EPQuiz recognises Reverse Recall Card'],
  [quiz,'reverse recall card','Recall type is driven by existing Question_Type'],
  [quiz,'Think of the Phrasal Verb first.','Recall prompt asks for independent retrieval before reveal'],
  [quiz,'View Answer','Recall card has explicit answer reveal'],
  [quiz,'✓ Yaad tha','Recall card exposes successful-recall self rating'],
  [quiz,'~ Confused','Recall card exposes partial/uncertain self rating'],
  [quiz,'✕ Bhool gaya','Recall card exposes forgotten self rating'],
  [quiz,'data-key="A"','Yaad tha maps to A'],
  [quiz,'data-key="B"','Confused maps to B'],
  [quiz,'data-key="C"','Bhool gaya maps to C'],
  [quiz,'q.options.forEach','Normal MCQ rendering path remains present'],
  [quiz,"EPApp.call('submitAnswer'",'Recall ratings reuse the existing central answer call'],
  [save,"OUTBOX_KEY='ep-answer-outbox-v3'",'Existing answer outbox remains authoritative'],
  [save,"direct('submitAnswerBatchV4'",'Outbox still drains through submitAnswerBatchV4'],
  [batch,"if(!['A','B','C','D'].includes(selected))",'Existing batch endpoint accepts A/B/C without a new save protocol'],
  [batch,'Selected_Answer:x.selected','Performance keeps selected A/B/C distinctly'],
  [batch,'Concept_ID:x.q.conceptId','Performance keeps Concept_ID'],
  [batch,'Attempt_ID:x.attemptId','Performance keeps stable Attempt_ID'],
  [pv,'phrasalQuestionFamilyV2_','Phrasal normalises Question_Type into learning families'],
  [pv,"return'recall'",'Reverse Recall is an independent-recall family'],
  [pv,"return'confusion'",'Confusion/discrimination has a separate family'],
  [pv,"return'recognition'",'Existing application/meaning questions remain recognition evidence'],
  [pv,'recallConfused','Confused evidence remains separately countable'],
  [pv,'recallForgotten','Forgotten evidence remains separately countable'],
  [pv,'recallWeak','Recognised-but-not-retrievable signal exists'],
  [pv,'phrasalPreferredFamilyV2_','Smart Phrasal chooses the learning family before variant rotation'],
  [pv,"if(e.recallWeak&&e.recognitionStrong)return'recall'",'Recognition-strong + Recall-weak prioritises a recall variant'],
  [pv,'phrasalChooseVariantV1_','Existing permanent Question_ID variant chooser remains authoritative'],
  [pv,"EP_PHRASAL_DAILY_TARGET=20",'Future Phrasal daily target is 20'],
  [pvui,'function dailyTarget(t)','Today UI derives the visible count from permanent batch data'],
  [pvui,'actual>0?actual','Actual permanent source count overrides the target label'],
  [nav,'Smart + Today’s 20','Home quick status reflects Today’s 20'],
  [nav,'Today’s 20 · Smart Revision','Library Phrasal entry reflects Today’s 20']
].forEach(([t,s,m])=>need(t,s,m));
...
function dateKeyFixture(d){const x=d instanceof Date?d:new Date(d);if(isNaN(x))return'';const p=n=>String(n).padStart(2,'0');return `${x.getUTCFullYear()}-${p(x.getUTCMonth()+1)}-${p(x.getUTCDate())}`;}
function intelligenceRegression(){
  try{
    const ctx={console,Date,Set,Math,dateKey_:dateKeyFixture};vm.createContext(ctx);vm.runInContext(learning,ctx,{filename:'LearningIntelligence.gs'});vm.runInContext(pv,ctx,{filename:'PhrasalMastery.gs'});
    ...
    const spaced=vm.runInContext("phrasalConceptMasteredV1_(learningProfileV2_([{ts:new Date('2026-08-01T00:00:00Z'),correct:true},{ts:new Date('2026-08-03T00:00:00Z'),correct:true},{ts:new Date('2026-08-05T00:00:00Z'),correct:true},{ts:new Date('2026-08-10T00:00:00Z'),correct:true}]))",ctx);
    if(!spaced)throw new Error('properly spaced four-checkpoint success did not become concept mastery');
    ok('Phrasal concept mastery follows central spaced-retention rules');
...
class Node{constructor(doc,id=''){this.doc=doc;this.id=id;this.className='';this.classList=new CL();this.style={};this.dataset={};this.disabled=false;this.textContent='';this.children=[];this.onclick=null;this._html='';this.recallButtons=[];this.optionText={textContent:''};if(id)doc.nodes[id]=this}set innerHTML(v){this._html=String(v||'');this.children=[];this.recallButtons=[];if(this._html.includes('recall-rate'))for(const k of ['A','B','C']){const b=new Node(this.doc);b.dataset.key=k;b.classList=new CL();this.recallButtons.push(b)}}get innerHTML(){return this._html}appendChild(n){this.children.push(n);return n}querySelector(sel){if(sel==='.option-text')return this.optionText;return null}querySelectorAll(sel){if(sel==='.recall-rate')return this.recallButtons;return[]}}
function quizEnv(){
  const doc={nodes:{},getElementById(id){return this.nodes[id]||null},createElement(){return new Node(this)},querySelectorAll(){return[]}};doc.head=new Node(doc,'head');doc.body=new Node(doc,'body');
  ...
  const sandbox={document:doc,window:null,localStorage,EPApp,console,Date,Math,JSON,Object,Array,String,Number,Boolean,RegExp,Set,Map,Promise,Error,confirm:()=>true,setTimeout:fn=>{fn();return 1}};sandbox.window=sandbox;sandbox.window.scrollTo=()=>{};vm.createContext(sandbox);vm.runInContext(strip(quiz),sandbox,{filename:'QuizJS.html'});const quizApi=vm.runInContext('EPQuiz',sandbox);return {doc,calls,quiz:quizApi};
}
async function recallUiRegression(){
  try{
    ...
    e.quiz.start([q],{mode:'phrasalDaily',batchId:'phrasal-today-2026-08-25'});let list=e.doc.nodes.optionList;
    ...
    list.recallButtons[1].onclick();await Promise.resolve();await Promise.resolve();const b=e.calls.find(x=>x.fn==='submitAnswer');if(!b||b.payload.selectedKey!=='B')throw new Error('Confused did not save as B through submitAnswer');
    ok('Reverse Recall reveal and A/B/C save path remain intact');
    const n=quizEnv(),mcq={id:'PV0001',topic:'Phrasal Verb',word:'Bear Away',question:'What does Bear Away mean?',questionType:'Meaning',correctKey:'B',options:[{key:'A',text:'A1'},{key:'B',text:'B1'},{key:'C',text:'C1'},{key:'D',text:'D1'}]};n.quiz.start([mcq],{mode:'phrasalRevision'});if(n.doc.nodes.optionList.children.length!==4)throw new Error('normal MCQ did not render four existing options');ok('Normal Phrasal MCQ path remains four-option');
