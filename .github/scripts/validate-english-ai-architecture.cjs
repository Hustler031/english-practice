...
const semantic=must('supabase/functions/english-concept-semantic/index.ts','english_semantic_claim');
...
for(const [name,text] of [['semantic',semantic],['context',context],['revision',revision]]){
  if(text.includes('"Access-Control-Allow-Origin": "*"'))throw new Error(`${name} worker advertises wildcard browser CORS`);
}
...
require('./validate-english-migration-reconciliation.cjs');
console.log('English AI architecture ownership, lifecycle, server-only CORS, and migration reconciliation contracts passed.');
