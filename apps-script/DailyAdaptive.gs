...
function getBootstrapV3(){
  ensurePerformanceModuleColumn_();ensureLearningStatusMigration_();const cfg=config_(),all=allQuestions_(),daily=ensureDailyAdaptiveV3_(all,Number(cfg.DAILY_TARGET||120)),actionable=daily.rows,mastered=currentMasteredMapV2_(),status=statusMap_(),counts={};all.forEach(q=>{if(!isActive_(q)||mastered[q.id])return;const id=canonicalCategory_(q.topic);counts[id]=(counts[id]||0)+1;});const categories=table_(EP.sheets.categories).filter(r=>truthy_(r.Active)).sort((a,b)=>Number(a.Display_Order||99)-Number(b.Display_Order||99)).map(r=>({id:r.Category_ID,name:r.Category_Name,parent:r.Parent_Category,home:truthy_(r.Home_Visible),count:Number(counts[r.Category_ID]||0)})),today=todayKey_(),hinduToday=table_(EP.sheets.hindu).filter(r=>truthy_(r.Active)&&dateKey_(r.Date)===today).length,recallIds=recallIds_(status),recall=all.filter(q=>isActive_(q)&&!mastered[q.id]&&recallIds.has(q.id)).length,masteredCount=Object.keys(mastered).filter(id=>mastered[id]).length;return {schemaVersion:4,dailyTarget:Number(cfg.DAILY_TARGET||120),extraCounts:String(cfg.EXTRA_COUNTS||'10,20,30,50').split(',').map(Number).filter(Number.isFinite),categories,dailyInfo:daily.info,stats:{dailyTotal:actionable.length,dailyCompleted:actionable.filter(r=>String(r.Status||'').toLowerCase()==='completed').length,hinduToday,recall,mastered:masteredCount,totalActive:all.filter(q=>isActive_(q)&&!mastered[q.id]).length}};
}
function getDailyBatchV3(){
  const all=allQuestions_(),daily=ensureDailyAdaptiveV3_(all,Number(config_().DAILY_TARGET||120)),rows=daily.rows,map=Object.fromEntries(all.map(q=>[q.id,q])),batchDate=rows.length?dateKey_(rows[0].Quiz_Date):todayKey_(),rationale=dailyRationaleSnapshotV4_(rows,all,batchDate),items=rationale.items||{};
  return rows.filter(r=>String(r.Status||'').toLowerCase()!=='completed').map(r=>{const id=String(r.Question_ID||'').trim(),q=map[id];const x=serveQuestion_(q),meta=items[id]||[String(r.Reason||'Mixed Revision'),''];x.selectionReason=String(meta[0]||r.Reason||'Mixed Revision');x.selectionReasonCode=dailyReasonCodeV4_(x.selectionReason);x.selectionSignals=String(meta[1]||'').split(',').filter(Boolean);return x;});
}
...
function dailyCurrentRowsV5_(rows,all,currentKey,shared){shared=shared||{};const map=Object.fromEntries((all||[]).map(q=>[q.id,q])),facts=shared.facts||performanceFactsV2_(),profiles=shared.profiles||learningProfilesV2_(facts),mastered=shared.mastered||currentMasteredMapV2_(),stars=shared.stars||currentStarredMapV2_(),diff=shared.diff||centralDifficultMapV2_(),key=currentKey||todayKey_();return (rows||[]).filter(r=>{if(String(r.Status||'').toLowerCase()==='completed')return true;const id=String(r.Question_ID||'').trim(),q=map[id],p=profiles[id]||learningProfileV2_([]);return !!(q&&isActive_(q)&&!mastered[id]&&dailyPrimaryReasonV5_(q,p,key,stars,diff));});}

function ensureDailyAdaptiveV3_(all,target){
  target=Math.max(1,Number(target||120));const today=todayKey_(),s=sheet_(EP.sheets.daily),mastered=currentMasteredMapV2_(),props=PropertiesService.getScriptProperties();let rows=table_(EP.sheets.daily).filter(r=>String(r.Question_ID||'').trim()),shared=rows.length?{facts:performanceFactsV2_(),mastered,profiles:null,stars:currentStarredMapV2_(),diff:centralDifficultMapV2_()}:null;if(shared)shared.profiles=learningProfilesV2_(shared.facts);rows=normalizeDailyRowsV3_(rows,all,mastered,s);rows=syncDailyCompletionsV3_(rows,s,shared&&shared.facts);
  ...
  rows=repairSkippedDailyDateV2_(rows,s,today);const batchDate=rows.length?dateKey_(rows[0].Quiz_Date):'',currentRows=dailyCurrentRowsV5_(rows,all,batchDate||today,shared);
...
function syncDailyCompletionsV3_(rows,s,facts){
  if(!rows.length)return rows;facts=facts||performanceFactsV2_();const byId=facts.byId||{};rows.forEach((r,i)=>{if(String(r.Status||'').toLowerCase()==='completed')return;const id=String(r.Question_ID||'').trim(),key=dateKey_(r.Quiz_Date);if(!id||!key)return;const p=key.split('-').map(Number),start=new Date(p[0],p[1]-1,p[2],0,0,0,0),hit=(byId[id]||[]).some(a=>a.ts>=start&&String(a.module||'').toLowerCase()==='daily');if(hit){s.getRange(i+2,5).setValue('Completed');r.Status='Completed';}});return rows;
}
