...
function starredRevisionIndex_(){
  ...
  const status=statusMap_(),all=allQuestions_(),qmap=Object.fromEntries(all.map(q=>[q.id,q])),difficult=starredRevisionDifficultMap_(),facts=learningFacts_(),events={},s=starredRevisionLogSheet_();
  ...
  Object.keys(qmap).forEach(id=>{
    const st=status[id]||{},ev=events[id],currentlyStarred=isStarredStatus_(st),mastered=!!st.mastered,ls=facts.byQuestion[id]||{state:'UNSEEN'};
    ...
    const q=qmap[id];rows.push({id,day,date:d,starred:active,mastered,difficult:!!difficult[id],weak:ls.state==='WEAK'||ls.state==='PERSISTENT_WEAK',persistentWeak:ls.state==='PERSISTENT_WEAK',state:ls.state,attempts:(facts.timeline.byId[id]||[]).length,word:q.word||'',question:q.question||'',topic:q.topic||'',source:q.sourceFile||q.sourceId||''});
  });
  return rows;
}
function setStarredRevisionDifficult(questionId,difficult){return setQuestionDifficult(questionId,difficult);}
