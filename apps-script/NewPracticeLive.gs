...
function newPracticeTypeName_(id){return({VOC:'Vocabulary',IDIOM:'Idioms & Phrases',PHRASAL:'Phrasal Verbs',OWS:'One Word Substitution',SPELL:'Spelling Mistakes',CU:'Concept / Usage',OTHER:'Other'})[id]||newPracticeCategoryName_(id,id)}
...
function newPracticePermanentPool_(){
  if(typeof reconcileReadyMyWordsFast_==='function'){try{reconcileReadyMyWordsFast_(false)}catch(e){}}
...
function newPracticeStats_(rows,status,stars){stars=stars||currentStarredMapV2_();return{total:rows.length,weak:rows.filter(q=>newPracticeWeak_(status[q.id]||{})).length,newCount:rows.filter(q=>Number((status[q.id]||{}).attempts||0)===0).length,starred:rows.filter(q=>!!stars[q.id]).length}}
function getNewPracticeHubLive(){
  const pool=newPracticePermanentPool_(),status=statusMap_(),stars=currentStarredMapV2_(),fixed=['VOC','IDIOM','PHRASAL','OWS','SPELL'],extra=[];pool.forEach(q=>{if(!fixed.includes(q.__npType)&&!extra.includes(q.__npType))extra.push(q.__npType)});
  const ids=fixed.concat(extra),categories=ids.map(id=>{const rows=pool.filter(q=>q.__npType===id),names=[...new Set(rows.map(q=>q.__npSource).filter(Boolean))].sort(),sources=names.map(name=>({name,...newPracticeStats_(rows.filter(q=>q.__npSource===name),status,stars)}));return{id,name:newPracticeTypeName_(id),...newPracticeStats_(rows,status,stars),sources}});
  return{categories,...newPracticeStats_(pool,status,stars)};
}
function getNewPracticeItemsLive(category,source){const status=statusMap_(),stars=currentStarredMapV2_();return newPracticeLivePool_(category||'ALL',source||'ALL').map(q=>({id:q.id,word:q.word||'',question:q.question||'',category:q.__npType||newPracticeType_(q),topic:q.topic||'',subtopic:q.subtopic||'',weak:newPracticeWeak_(status[q.id]||{}),starred:!!stars[q.id],attempts:Number((status[q.id]||{}).attempts||0),source:q.__npSource||'',added:(()=>{const d=recentContentDate_(q);return d?Utilities.formatDate(d,Session.getScriptTimeZone(),'yyyy-MM-dd'):''})()}))}
function getNewPracticeBatchLive(category,kind,count,source){const status=statusMap_(),stars=currentStarredMapV2_(),diff=centralDifficultMapV2_(),mode=String(kind||'all').toLowerCase();let pool=newPracticeLivePool_(category||'ALL',source||'ALL'),requested=Math.max(1,Math.min(100,Number(count||10)));if(mode==='new'||mode==='newwords')pool=newPracticeNewWordOrder_(pool,status).slice(0,requested);else if(mode==='starred')pool=newPracticeNewWordOrder_(pool.filter(q=>!!stars[q.id]),status).slice(0,requested);else if(mode==='weak'){pool=pool.filter(q=>newPracticeWeak_(status[q.id]||{}));shuffle_(pool);if(Number(count||0)>0)pool=pool.slice(0,requested)}else if(mode==='random'){shuffle_(pool);pool=pool.slice(0,requested)}else if(Number(count||0)>0)pool=pool.slice(0,Math.min(120,Number(count)));return serveQuestionsCentralV3_(pool,{stars,diff})}
