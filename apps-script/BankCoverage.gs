...
function bankCoverageMetaV3_(g,s,diff){const facts=s.facts,ids=[...new Set(g.questions.map(q=>q.id))],exp=ids.filter(id=>facts.seen.has(id)).length,unseenIds=ids.filter(id=>!facts.seen.has(id)&&!s.mastered[id]),unseen=unseenIds.length,catIds=new Set(ids),doneToday=new Set(facts.all.filter(a=>a.module==='bankCoverage'&&dateKey_(a.ts)===s.today&&catIds.has(a.id)).map(a=>a.id)).size,available=Math.max(0,Math.min(10-doneToday,unseen));return {id:g.id,name:g.name,total:ids.length,exposed:exp,unseen,available,doneToday,coverage:learningPctV2_(exp,ids.length),difficult:ids.filter(id=>!!diff[id]).length,complete:unseen===0};}
...
function getBankCoverageBatch(category){
  const cat=String(category||'').trim(),s=bankCoverageSnapshotV3_(),facts=s.facts,group=s.rows.find(g=>g.id===cat);if(!group)return [];const diff=centralDifficultMapV2_(),stars=currentStarredMapV2_(),meta=bankCoverageMetaV3_(group,s,diff);if(meta.available<=0)return [];
  const pool=shuffle_(group.questions.filter(q=>!s.mastered[q.id]&&!facts.seen.has(q.id))).slice(0,meta.available);return pool.map(q=>{const x=serveQuestion_(q);x.difficult=!!diff[q.id];x.marked=!!stars[q.id];return x;});
}
