...
function learningProfileV2_(attempts){
  ...
  const correct=attempts.filter(a=>a.correct).length,wrong=n-correct,firstCorrect=!!attempts[0].correct;let retentionAttempts=0,retentionCorrect=0,retentionWrong=0,afterReviewAttempts=0,afterReviewCorrect=0,streak=0;const spaced=[];
  attempts.forEach((a,i)=>{if(a.correct)streak++;else streak=0;if(i===0)return;const gap=a.ts-attempts[i-1].ts;if(gap>=EP_RETENTION_GAP_MS){retentionAttempts++;spaced.push(a);if(a.correct)retentionCorrect++;else retentionWrong++;}else{afterReviewAttempts++;if(a.correct)afterReviewCorrect++;}});
  const retentionAccuracy=retentionAttempts?retentionCorrect/retentionAttempts:null,afterReviewAccuracy=afterReviewAttempts?afterReviewCorrect/afterReviewAttempts:null,lastCorrect=!!attempts[n-1].correct,recentSpacedWrong=spaced.slice(-3).filter(a=>!a.correct).length;
  const persistent=wrong>=3||recentSpacedWrong>=2;let state='Learning';
  if(persistent)state='Persistent Weak';
  else if(!lastCorrect)state='Weak';
  else if(wrong>0&&(retentionCorrect<2||retentionAttempts<2||recentSpacedWrong>0))state='Fragile';
  else if(n===1)state='Learning';
  else if(retentionCorrect>=2&&retentionAccuracy!==null&&retentionAccuracy>=.8&&recentSpacedWrong===0)state='Strong';
  else if(retentionAttempts===0)state='Fragile';
  else if(retentionAccuracy!==null&&retentionAccuracy<.7)state='Weak';
  else state='Fragile';
  const lastTwoSpaced=spaced.slice(-2),provenMastery=n>=3&&retentionCorrect>=2&&retentionAccuracy!==null&&retentionAccuracy>=.8&&lastCorrect&&lastTwoSpaced.length>=2&&lastTwoSpaced.every(a=>a.correct);
