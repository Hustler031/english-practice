...
function phrasalConceptsV1_(){
  const qs=phrasalQuestionsV1_(),facts=performanceFactsV2_(),status=statusMap_(),stars=currentStarredMapV2_(),diff=centralDifficultMapV2_(),mastered=currentMasteredMapV2_(),now=new Date(),groups={};
  ...
  return Object.values(groups).map(g=>{
    const ids=g.questions.map(q=>q.id),attempts=ids.flatMap(id=>facts.byId[id]||[]).sort((a,b)=>a.ts-b.ts),profile=learningProfileV2_(attempts),moduleAttempts=attempts.filter(a=>EP_PHRASAL_MODULES.has(String(a.module||'').toLowerCase())),lastRevision=moduleAttempts.length?moduleAttempts[moduleAttempts.length-1].ts:null,activeVariants=g.questions.filter(q=>isActive_(q)&&!mastered[q.id]),allMastered=g.questions.length>0&&g.questions.every(q=>!!mastered[q.id]),due=activeVariants.some(q=>{const n=status[q.id]&&status[q.id].nextReview?new Date(status[q.id].nextReview):null;return !!(n&&!isNaN(n)&&n<=now)}),starred=activeVariants.some(q=>!!stars[q.id]),difficult=activeVariants.some(q=>!!diff[q.id]);
    return Object.assign(g,{ids,attempts,profile,activeVariants,mastered:allMastered,due,starred,difficult,revisedCount:moduleAttempts.length,neverRevised:moduleAttempts.length===0,lastRevision,daysSinceRevision:phrasalDaysSinceV1_(lastRevision,now)});
  });
}
function phrasalTierV1_(x){const s=String(x.profile&&x.profile.state||'New');if(s==='Persistent Weak')return 9;if(s==='Weak')return 8;if(s==='Fragile')return 7;if(x.due)return 6;if(x.difficult)return 5;if(x.starred)return 4;if(x.neverRevised)return 3;if(s==='Learning'||s==='New')return 2;return 1;}
...
function phrasalRotationSortV1_(a,b){if(Number(b.neverRevised)!==Number(a.neverRevised))return Number(b.neverRevised)-Number(a.neverRevised);const at=a.lastRevision instanceof Date?a.lastRevision.getTime():0,bt=b.lastRevision instanceof Date?b.lastRevision.getTime():0;if(at!==bt)return at-bt;return phrasalLearningSortV1_(a,b);}
function phrasalReasonV1_(x,lane){const s=String(x.profile&&x.profile.state||'New');if(lane==='rotation'){if(x.neverRevised)return'Never / Under-revised';if(Number(x.daysSinceRevision||0)>=7)return'Longest Not Seen';return'Healthy Rotation';}if(['Persistent Weak','Weak','Fragile'].includes(s))return s;if(x.due)return'Due Retention';if(x.difficult&&x.starred)return'Difficult + Starred';if(x.difficult)return'Difficult';if(x.starred)return'Starred';if(s==='Learning'||s==='New')return'Learning';return'Rotation';}
function phrasalMarkV1_(x,lane){return Object.assign({},x,{selectionLane:lane,selectionReason:phrasalReasonV1_(x,lane)});}
function phrasalEligibleConceptsV1_(concepts){return (concepts||[]).filter(x=>x.activeVariants.length&&!x.mastered);}
...
function getPhrasalTodayBatchV1(){const rows=phrasalDailyRowsV1_(todayKey_()),maps=centralServeMapsV3_();return serveQuestionsCentralV3_(rows.filter(q=>isActive_(q)&&!currentMasteredMapV2_()[q.id]),maps);}
function phrasalHistoryV1_(){
  const facts=performanceFactsV2_(),batches={};phrasalQuestionsV1_().forEach(q=>{const date=phrasalSourceDateV1_(q);if(!date)return;if(!batches[date])batches[date]={date,generated:0,questionIds:new Set(),practised:new Set()};batches[date].generated++;batches[date].questionIds.add(q.id)});
  Object.values(batches).forEach(b=>b.questionIds.forEach(id=>(facts.byId[id]||[]).filter(a=>String(a.module||'').toLowerCase()==='phrasaldaily').forEach(()=>b.practised.add(id))));
  const ordered=Object.values(batches).sort((a,b)=>a.date.localeCompare(b.date)).map((b,i)=>({day:i+1,date:b.date,label:b.date===todayKey_()?'Today':'Day '+(i+1),generated:b.generated,practised:b.practised.size}));if(!ordered.length)return[];const currentDay=ordered.length,currentMonth=Math.floor((currentDay-1)/30)+1,currentMonthStart=(currentMonth-1)*30+1,currentBlockStart=Math.floor((currentDay-1)/10)*10+1,out=[];
