...
- Train both recognition/application and independent retrieval. Reverse Recall Card variants use the existing Question_Type field.
- When the MCQ format permits, use at least three plausible competing distractors, preferably related phrasal verbs,
...
function phrasalConceptMasteredV1_(profile){return !!(profile&&profile.provenMastery)&&String(profile&&profile.state||'')==='Proven Mastered';}
function phrasalQuestionFamilyV2_(q){const t=String(q&&q.questionType||'').trim().toLowerCase();if(/reverse\s+recall\s+card/.test(t))return'recall';if(/incorrect|confus|discrimin/.test(t))return'confusion';return'recognition';}
function phrasalFamilyStatsV2_(attempts){const a=(attempts||[]).slice().sort((x,y)=>x.ts-y.ts),n=a.length,correct=a.filter(x=>x.correct).length,last=n?a[n-1]:null;return {attempts:n,correct,wrong:n-correct,accuracy:n?correct/n:null,lastCorrect:last?!!last.correct:null,lastSelected:last?String(last.selected||'').toUpperCase():'',lastAttempt:last?last.ts:null};}
function phrasalConceptTypeEvidenceV2_(questions,facts){
  const lanes={recognition:[],recall:[],confusion:[]};(questions||[]).forEach(q=>{const f=phrasalQuestionFamilyV2_(q);(facts.byId[q.id]||[]).forEach(a=>lanes[f].push(a))});
  const recognition=phrasalFamilyStatsV2_(lanes.recognition),recall=phrasalFamilyStatsV2_(lanes.recall),confusion=phrasalFamilyStatsV2_(lanes.confusion),recallSelected=lanes.recall.map(a=>String(a.selected||'').toUpperCase()),recallSuccess=recallSelected.filter(x=>x==='A').length,recallConfused=recallSelected.filter(x=>x==='B').length,recallForgotten=recallSelected.filter(x=>x==='C').length;
  const recognitionStrong=recognition.attempts>=2&&recognition.accuracy!==null&&recognition.accuracy>=.75&&recognition.lastCorrect===true,recognitionWeak=recognition.attempts>0&&(recognition.lastCorrect===false||(recognition.accuracy!==null&&recognition.accuracy<.7)),recallWeak=recall.attempts>0&&(['B','C'].includes(recall.lastSelected)||(recallSuccess/recall.attempts)<.67),confusionWeak=confusion.attempts>0&&(confusion.lastCorrect===false||(confusion.accuracy!==null&&confusion.accuracy<.7));
  return {recognition,recall,confusion,recognitionStrong,recognitionWeak,recallWeak,confusionWeak,recallSuccess,recallConfused,recallForgotten,needsRecallExposure:recognitionStrong&&recall.attempts===0};
}
function phrasalPreferredFamilyV2_(concept){const e=concept&&concept.typeEvidence||{};if(e.recallWeak&&e.recognitionStrong)return'recall';if(e.recognitionWeak)return e.confusionWeak?'confusion':'recognition';if(e.recallWeak)return'recall';if(e.confusionWeak&&e.recall&&e.recall.lastCorrect===true)return'confusion';if(e.needsRecallExposure)return'recall';return'';}
function phrasalConceptsV1_(){
  const qs=phrasalQuestionsV1_(),facts=performanceFactsV2_(),stars=currentStarredMapV2_(),diff=centralDifficultMapV2_(),questionMastered=currentMasteredMapV2_(),now=new Date(),today=todayKey_(),groups={};
  ...
  return Object.values(groups).map(g=>{
    const ids=g.questions.map(q=>q.id),attempts=ids.flatMap(id=>facts.byId[id]||[]).sort((a,b)=>a.ts-b.ts),profile=learningProfileV2_(attempts),typeEvidence=phrasalConceptTypeEvidenceV2_(g.questions,facts),moduleAttempts=attempts.filter(a=>EP_PHRASAL_MODULES.has(String(a.module||'').toLowerCase())),lastRevision=moduleAttempts.length?moduleAttempts[moduleAttempts.length-1].ts:null,activeVariants=g.questions.filter(q=>isActive_(q)&&!questionMastered[q.id]),freshVariants=activeVariants.filter(q=>!(facts.byId[q.id]||[]).length),conceptMastered=phrasalConceptMasteredV1_(profile),due=learningDueV3_(profile,today),starred=activeVariants.some(q=>!!stars[q.id]),difficult=activeVariants.some(q=>!!diff[q.id]);
    return Object.assign(g,{ids,attempts,profile,typeEvidence,recallWeak:!!typeEvidence.recallWeak,recognitionStrong:!!typeEvidence.recognitionStrong,activeVariants,freshVariants,freshVariantCount:freshVariants.length,questionMasteredCount:g.questions.filter(q=>!!questionMastered[q.id]).length,mastered:conceptMastered,due,starred,difficult,revisedCount:moduleAttempts.length,neverRevised:moduleAttempts.length===0,lastRevision,daysSinceRevision:phrasalDaysSinceV1_(lastRevision,now)});
  });
}
function phrasalTierV1_(x){const s=String(x.profile&&x.profile.state||'New');if(x.due&&s==='Persistent Weak')return 10;if(x.due&&x.recallWeak&&x.recognitionStrong)return 9;if(x.due&&s==='Weak')return 8;if(x.due&&s==='Fragile')return 7;if(x.due)return 6;if(x.difficult)return 5;if(x.starred)return 4;if(x.mastered&&x.freshVariantCount)return 3;if(x.neverRevised)return 3;if(s==='Learning'||s==='New')return 2;return 1;}
...
function phrasalSelectConceptsV1_(concepts,mode,count){
  ...
  if(m==='weak')pool=pool.filter(x=>['Persistent Weak','Weak','Fragile'].includes(String(x.profile&&x.profile.state||''))||x.recallWeak).sort(phrasalLearningSortV1_);
...
function phrasalChooseVariantV1_(concept,facts,mastered){
  const candidates=(concept.activeVariants||[]).filter(q=>!mastered[q.id]);if(!candidates.length)return null;const preferred=phrasalPreferredFamilyV2_(concept);
  return candidates.slice().sort((a,b)=>{const af=phrasalQuestionFamilyV2_(a),bf=phrasalQuestionFamilyV2_(b),ap=preferred&&af===preferred?0:1,bp=preferred&&bf===preferred?0:1;if(ap!==bp)return ap-bp;const aa=facts.byId[a.id]||[],bb=facts.byId[b.id]||[],am=aa.filter(x=>EP_PHRASAL_MODULES.has(String(x.module||'').toLowerCase())),bm=bb.filter(x=>EP_PHRASAL_MODULES.has(String(x.module||'').toLowerCase()));if(am.length!==bm.length)return am.length-bm.length;const at=am.length?am[am.length-1].ts.getTime():0,bt=bm.length?bm[bm.length-1].ts.getTime():0;if(at!==bt)return at-bt;if(aa.length!==bb.length)return aa.length-bb.length;return String(a.id).localeCompare(String(b.id));})[0];
}
// Phrasal serving preserves the same central Starred/Difficult flag semantics as serveQuestionsCentralV3_.
function phrasalServeQuestionV2_(base,maps){const q=serveQuestion_(base);maps=maps||centralServeMapsV3_();q.marked=!!maps.stars[base.id];q.difficult=!!maps.diff[base.id];q.questionType=String(base.questionType||'');q.conceptId=phrasalConceptKeyV1_(base);q.phrasalQuestionFamily=phrasalQuestionFamilyV2_(base);return q;}
function phrasalServeConceptsV1_(selected){const facts=performanceFactsV2_(),mastered=currentMasteredMapV2_(),maps=centralServeMapsV3_();return (selected||[]).map(x=>{const base=phrasalChooseVariantV1_(x,facts,mastered);if(!base)return null;const q=phrasalServeQuestionV2_(base,maps);q.phrasalConceptId=x.conceptId;q.phrasalSelectionReason=x.selectionReason;q.phrasalSelectionLane=x.selectionLane;q.phrasalBaselineState=String(x.profile&&x.profile.state||'New');q.phrasalConceptMastered=!!x.mastered;q.phrasalLearningNeed=phrasalPreferredFamilyV2_(x)||'rotation';q.phrasalRecallWeak=!!x.recallWeak;return q;}).filter(Boolean);}
...
function getPhrasalTodayBatchV1(){const rows=phrasalDailyRowsV1_(todayKey_()),maps=centralServeMapsV3_(),mastered=currentMasteredMapV2_();return rows.filter(q=>isActive_(q)&&!mastered[q.id]).map(q=>phrasalServeQuestionV2_(q,maps));}
...
function getPhrasalHistoryBatchV1(fromDay,toDay){const rows=phrasalHistoryScopeV1_(phrasalHistoryOrderedV1_(),fromDay,toDay),mastered=currentMasteredMapV2_(),maps=centralServeMapsV3_();return rows.filter(q=>isActive_(q)&&!mastered[q.id]).map(q=>phrasalServeQuestionV2_(q,maps));}
...
function phrasalStatsV1_(concepts){const all=concepts||[],eligible=phrasalEligibleConceptsV1_(all),exposed=all.filter(x=>x.attempts.length>0).length;return {totalConcepts:all.length,exposed,exposurePercent:all.length?Math.round(exposed*1000/all.length)/10:0,due:eligible.filter(x=>x.due).length,weak:eligible.filter(x=>['Persistent Weak','Weak','Fragile'].includes(String(x.profile&&x.profile.state||''))||x.recallWeak).length,recallWeak:eligible.filter(x=>x.recallWeak).length,difficult:eligible.filter(x=>x.difficult).length,starred:eligible.filter(x=>x.starred).length,mastered:all.filter(x=>x.mastered).length,freshVariantChecks:eligible.filter(x=>x.mastered&&x.freshVariantCount).length,eligible:eligible.length};}
function getPhrasalMasteryHubV1(){const c=phrasalConceptsV1_(),stats=phrasalStatsV1_(c),today=phrasalDailyRowsV1_(todayKey_());return {version:EP_PHRASAL_VERSION,generatedAt:new Date().toISOString(),dailyTarget:EP_PHRASAL_DAILY_TARGET,stats,today:{date:todayKey_(),count:today.length,target:EP_PHRASAL_DAILY_TARGET,ready:today.length>0,sourceId:phrasalDailySourceIdV1_(todayKey_())},available:{smart:stats.eligible,weak:stats.weak,difficult:stats.difficult,starred:stats.starred,random:stats.eligible,all:stats.eligible},sizes:[10,20,30,50],history:phrasalHistoryV1_()};}
...
function getPhrasalMasteryAuditV1(){const c=phrasalConceptsV1_(),seen=new Set(),dupes=[];c.forEach(x=>{if(seen.has(x.conceptId))dupes.push(x.conceptId);seen.add(x.conceptId)});const today=phrasalDailyRowsV1_(todayKey_()),todayConcepts=today.map(phrasalConceptKeyV1_),todayDup=todayConcepts.filter((x,i)=>todayConcepts.indexOf(x)!==i);return {ok:dupes.length===0&&todayDup.length===0,questions:phrasalQuestionsV1_().length,concepts:c.length,duplicateConcepts:dupes,todayCount:today.length,todayDuplicateConcepts:[...new Set(todayDup)],stats:phrasalStatsV1_(c),typeIntelligence:{recallWeak:c.filter(x=>x.recallWeak).map(x=>x.conceptId),recognitionStrongRecallWeak:c.filter(x=>x.recallWeak&&x.recognitionStrong).map(x=>x.conceptId),recallConfused:c.reduce((n,x)=>n+Number(x.typeEvidence&&x.typeEvidence.recallConfused||0),0),recallForgotten:c.reduce((n,x)=>n+Number(x.typeEvidence&&x.typeEvidence.recallForgotten||0),0)}};}
