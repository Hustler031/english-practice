...
function getMySavedHubCentralV3(){const status=statusMap_(),stars=currentStarredMapV2_(),all=allQuestions_(),starred=all.filter(q=>isActive_(q)&&!(status[q.id]&&status[q.id].mastered)&&!!stars[q.id]),weak=starred.filter(q=>isWeakStatus_(status[q.id]||{})).length,words=getMyWords();return {starred:starred.length,starredWeak:weak,words:words.length,wordsReady:words.filter(x=>String(x.gptStatus||'').toLowerCase()==='ready').length,wordsAdded:words.filter(x=>x.practiceQuestionId||String(x.status).toLowerCase()==='added').length};}
function getStarredQuestionListCentralV3(){const status=statusMap_(),stars=currentStarredMapV2_();return allQuestions_().filter(q=>isActive_(q)&&!(status[q.id]&&status[q.id].mastered)&&!!stars[q.id]).map(q=>({id:q.id,word:q.word||'',question:q.question||'',category:canonicalCategory_(q.topic),topic:q.topic||'',weak:isWeakStatus_(status[q.id]||{}),attempts:Number((status[q.id]||{}).attempts||0),source:q.sourceFile||q.sourceId||''}));}
function getStarredPracticeBatchCentralV3(kind,count){const status=statusMap_(),maps=centralServeMapsV3_(),mode=String(kind||'all').toLowerCase();let pool=allQuestions_().filter(q=>isActive_(q)&&!(status[q.id]&&status[q.id].mastered)&&!!maps.stars[q.id]);if(mode==='weak')pool=pool.filter(q=>isWeakStatus_(status[q.id]||{}));if(mode==='random'||mode==='weak')shuffle_(pool);const requested=Math.max(1,Math.min(100,Number(count||10)));if(mode==='random'||(mode==='weak'&&Number(count||0)>0))pool=pool.slice(0,requested);return serveQuestionsCentralV3_(pool,maps);}
