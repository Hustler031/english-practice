...
function repairLearningDerivedV3_(id,module){
  id=String(id||'').trim();const q=findQuestion_(id);if(!q)throw new Error('Question not found for derived repair');const facts=performanceFactsV2_(),profile=learningProfileV2_(facts.byId[id]||[]);writeStatusSummaryV2_(q,profile,undefined,facts);if(String(module||'').toLowerCase()==='daily')markDaily_(id);try{PropertiesService.getScriptProperties().deleteProperty(EP_LEARNING_PROGRESS_KEY)}catch(e){}return {ok:true,id,status:profile.state};
}
