const EP_SMART_MY_SAVED_VERSION='V1';
...
function smartMySavedUniverseV1_(){
  const base=mySavedRevisionItemsV2_(),facts=performanceFactsV2_(),status=statusMap_(),stars=currentStarredMapV2_(),diff=centralDifficultMapV2_(),mastered=currentMasteredMapV2_(),now=new Date();
  const rows=base.filter(x=>x.active).map(x=>{const attempts=facts.byId[x.id]||[],profile=learningProfileV2_(attempts),moduleAttempts=attempts.filter(a=>String(a.module||'').toLowerCase()===EP_SMART_MY_SAVED_MODULE.toLowerCase()),lastRevision=moduleAttempts.length?moduleAttempts[moduleAttempts.length-1].ts:null,next=status[x.id]&&status[x.id].nextReview?new Date(status[x.id].nextReview):null,due=!!(next&&!isNaN(next)&&next<=now);return Object.assign({},x,{profile,starred:!!stars[x.id],difficult:!!diff[x.id],mastered:!!mastered[x.id],due,revisedCount:moduleAttempts.length,neverRevised:moduleAttempts.length===0,lastRevision,daysSinceRevision:smartMySavedDaysSinceV1_(lastRevision,now)});});
  return {rows,facts,now};
}
