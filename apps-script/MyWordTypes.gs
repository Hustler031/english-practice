...
function getMyWordPracticeTypeMap_(){const bySaved=getMyWordCaptureTypes(),out={};getMyWords().forEach(w=>{const qid=String(w.practiceQuestionId||'').trim();if(qid)out[qid]=normalizeMyWordType_(bySaved[String(w.id||'')]||'V')});return out;}
