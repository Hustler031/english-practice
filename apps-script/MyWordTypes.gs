...
function getMyWordCaptureType_(savedId){const id=String(savedId||'').trim(),types=getMyWordCaptureTypes();return normalizeMyWordType_(types[id]||'V');}
...
function myWordPracticeSpec_(type){const t=normalizeMyWordType_(type);return({V:{topic:'Vocabulary',questionType:'Meaning'},SM:{topic:'Spelling Mistakes',questionType:'Spelling'},OWS:{topic:'One Word Substitution',questionType:'One Word Substitution'},PV:{topic:'Phrasal Verbs',questionType:'Meaning'},IP:{topic:'Idioms & Phrases',questionType:'Meaning'}})[t]||{topic:'Vocabulary',questionType:'Meaning'};}
