...
export default function TeacherLibraryPage(){
 ...
 useEffect(()=>{if(!ready)return;const p=new URLSearchParams(window.location.search);setSeries(p.get("series")||"");let live=true;const unsub=subscribeGkFresh<Library>("gk_get_teacher_library",undefined,x=>{if(live&&x?.ok)setData(x)});gkRpc<Library>("gk_get_teacher_library").then(x=>{if(live&&x?.ok)setData(x)}).catch(()=>{if(live)setError("Teacher library could not be refreshed.")});return()=>{live=false;unsub();};},[ready]);
