...
export default function GkQuiz(){
 ...
 const[meta,setMeta]=useState<Meta>({lane:queryLane,mode:queryMode,source:querySource,title:queryTitle});const[qs,setQs]=useState<Q[]>([]);const[index,setIndex]=useState(0);const[answers,setAnswers]=useState<Answers>({});const[error,setError]=useState(false);const[loading,setLoading]=useState(true);const[noteOpen,setNoteOpen]=useState(false);const[note,setNote]=useState("");const[pauseOpen,setPauseOpen]=useState(false);
 useEffect(()=>{if(!ready)return;let alive=true;const stored=readGkPaused();const exactResume=!!stored&&(params.get("resume")==="1"||(stored.query===window.location.search&&stored.lane===queryLane&&stored.mode===queryMode));if(exactResume&&stored){setMeta({lane:stored.lane,mode:stored.mode,source:"resume",title:stored.title});setQs(stored.questions as Q[]);setIndex(Math.min(stored.index,stored.questions.length-1));setAnswers(stored.answers||{});setLoading(false);return;}const count=Math.max(1,Math.min(100,Number(params.get("count")||50)));let call:Promise<Q[]>;if(querySource==="lecture")call=gkRpc<Q[]>("gk_get_lecture_batch",{p_lecture_key:params.get("lecture")||"",p_lane:queryLane,p_mode:queryMode,p_count:count});else if(querySource==="subject")call=gkRpc<Q[]>("gk_get_subject_batch",{p_subject:params.get("subject")||"Unclassified",p_topic:params.get("topic")||null,p_lane:queryLane,p_mode:queryMode,p_count:count});else call=gkRpc<Q[]>("gk_get_lane_batch",{p_lane:queryLane,p_mode:queryMode,p_count:count});call.then(x=>{if(alive){setQs(Array.isArray(x)?x:[]);setLoading(false);}}).catch(()=>{if(alive){setError(true);setLoading(false);}});return()=>{alive=false;};},[ready,params,queryLane,queryMode,querySource]);
 useEffect(()=>{if(!qs.length||loading||typeof window==="undefined")return;saveGkPaused({title:meta.title,lane:meta.lane,mode:meta.mode,index,questions:qs,answers,query:window.location.search,savedAt:Date.now()});},[qs,index,answers,loading,meta]);
 ...
 function exitPaused(){if(qs.length&&typeof window!=="undefined")saveGkPaused({title:meta.title,lane:meta.lane,mode:meta.mode,index,questions:qs,answers,query:window.location.search,savedAt:Date.now()});window.location.href="/gk";}
 ...
 if(!q)return <main className={styles.quiz}><div className={styles.top}><button className={styles.back} onClick={()=>window.location.href="/gk"}>‹ GK</button></div><div className={styles.notice}>{error?"GK questions couldn’t load. Reopen this set to retry.":"No eligible questions are available in this set right now."}</div></main>;
