...
export default function GkQuiz(){
 const ready=useAuthGuard();const[qs,setQs]=useState<Q[]>([]);const[index,setIndex]=useState(0);const[answers,setAnswers]=useState<Answers>({});const[error,setError]=useState(false);const[loading,setLoading]=useState(true);const[noteOpen,setNoteOpen]=useState(false);const[note,setNote]=useState("");
 const params=useMemo(()=>typeof window==="undefined"?new URLSearchParams():new URLSearchParams(window.location.search),[]);
 const lane=(params.get("lane")==="RAPID"?"RAPID":"MAIN") as "MAIN"|"RAPID";const mode=params.get("mode")||"all";const source=params.get("source")||"lane";const title=params.get("title")|| (lane==="RAPID"?"Rapid Recall":"Main");
 useEffect(()=>{if(!ready)return;let alive=true;const paused=params.get("resume")==="1"?readGkPaused():null;if(paused){setQs(paused.questions as Q[]);setIndex(Math.min(paused.index,paused.questions.length-1));setAnswers(paused.answers||{});setLoading(false);return;}const count=Math.max(1,Math.min(100,Number(params.get("count")||50)));let call:Promise<Q[]>;if(source==="smart")call=gkRpc("gk_get_smart_revision",{p_mode:mode,p_count:count});else if(source==="lecture")call=gkRpc("gk_get_lecture_batch",{p_lecture_key:params.get("lecture")||"",p_lane:lane,p_mode:mode,p_count:count});else if(source==="subject")call=gkRpc("gk_get_subject_batch",{p_subject:params.get("subject")||"Unclassified",p_topic:params.get("topic")||null,p_lane:lane,p_mode:mode,p_count:count});else call=gkRpc("gk_get_lane_batch",{p_lane:lane,p_mode:mode,p_count:count});call.then(x=>{if(alive){setQs(Array.isArray(x)?x:[]);setLoading(false);}}).catch(()=>{if(alive){setError(true);setLoading(false);}});return()=>{alive=false;};},[ready,lane,mode,params,source]);
 const q=qs[index],answered=q?answers[q.id]:undefined;
 useEffect(()=>{setNote(q?.state?.note||"");setNoteOpen(false);},[q?.id]);
 async function answer(key:string){if(!q||answered)return;const correct=key===q.correctKey;setAnswers(a=>({...a,[q.id]:{selected:key,correct,correctKey:q.correctKey}}));try{await gkRpc("gk_submit_answer",{p_question_id:q.id,p_selected_option:key,p_marked_review:!!q.state?.starred,p_mode:`${lane.toLowerCase()}:${mode}`});}catch{setError(true);}}
 ...
 function pause(){if(!q)return;if(!window.confirm("Pause this GK practice and return home? Your exact position will be saved."))return;saveGkPaused({title,lane,mode,index,questions:qs,answers,query:window.location.search,savedAt:Date.now()});window.location.href="/gk";}
 function go(next:number){const n=Math.max(0,Math.min(qs.length-1,next));setIndex(n);window.scrollTo({top:0,behavior:"smooth"});if(n===qs.length-1&&next>=qs.length)clearGkPaused();}
 if(!ready)return <main className={styles.quiz}><div className="loading-copy">Checking session…</div></main>;
 if(loading)return <main className={styles.quiz}><div className="loading-copy">Loading {title}…</div></main>;
 ...
 return <main className={styles.quiz}>
  ...
  {answered&&<><section className={styles.explain}><h3>{answered.correct?"Correct · lock the fact":"Review · correct the recall"}</h3><p>{q.explanation||"Answer recorded."}</p>{q.trick&&<p><strong>Trick:</strong> {q.trick}</p>}{q.related_fact&&<p><strong>Related fact:</strong> {q.related_fact}</p>}{q.exam_trap&&<p><strong>Exam trap:</strong> {q.exam_trap}</p>}</section><div className={styles.tools}><button className={`${styles.tool} ${q.state?.starred?styles.activeTool:""}`} onClick={toggleStar}>★ Star</button><button className={`${styles.tool} ${q.state?.difficult?styles.activeTool:""}`} onClick={toggleDifficult}>◆ Difficult</button><button className={`${styles.tool} ${(q.state?.note||note)?styles.activeTool:""}`} onClick={()=>setNoteOpen(x=>!x)}>▤ Note</button></div>{noteOpen&&<section className={styles.explain}><h3>Personal note</h3><textarea className={styles.note} value={note} onChange={e=>setNote(e.target.value)} placeholder="Write the exact fact you want to remember"/><button className="btn primary" onClick={saveNote}>Save note</button></section>}</>}
  <footer className={styles.dock}><div className={styles.dockInner}><button onClick={()=>go(index-1)} disabled={index===0}>‹ Previous</button><button onClick={pause}>Ⅱ Pause</button><button onClick={()=>go(index+1)} disabled={index===qs.length-1}>Next ›</button></div></footer>
 </main>;
}
