...
export default function DailyAnalysisQuestionsPage(){
 ...
 return <main className="top-level-parity learner-rebuild-page learner-insights-page daily-analysis-page daily-analysis-list-page">
  ...
  {loading?<div className="loading-copy">Loading questions…</div>:data?.questions?.length?<section className="daily-analysis-question-list">{data.questions.map(row=><Link key={row.questionId} className="daily-analysis-question-row" href={`/english/revision/ai-intelligence/daily-analysis/review?category=${encodeURIComponent(category)}&questionId=${encodeURIComponent(row.questionId)}`}>
