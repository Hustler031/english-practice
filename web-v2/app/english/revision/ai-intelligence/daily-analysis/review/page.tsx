...
export default function DailyAnalysisReviewPage(){
 ...
 const backHref=category?`/english/revision/ai-intelligence/daily-analysis/questions?category=${encodeURIComponent(category)}`:"/english/revision/ai-intelligence/daily-analysis";
 return <main className="top-level-parity learner-rebuild-page learner-insights-page daily-analysis-page daily-analysis-detail-page">
  <PageHeader back={<Link href={backHref} className="back-link">← {meta?.title||"Daily Analysis"}</Link>} eyebrow="Read-only review" title={a?.displayName||"Question review"} subtitle="Correct answer is shown so you can inspect the weakness manually."/>
