...
export default function FastTrackPage(){
 ...
 return <section className="route-page">
  <div className="route-head"><Link className="btn ghost" href="/english/revision">← Revision</Link><div><span className="eyebrow">Cheap verification · Central Intelligence</span><h1>Fast Track Mastery</h1><p>I probably know this — verify it cheaply.</p></div></div>
  {error&&<div className="error-box">{error}</div>}
  <section className="route-summary"><Metric label="Ready" value={ft?.readyToVerify??"—"}/><Metric label="Mastered" value={ft?.mastered??"—"}/><Metric label="Remaining" value={ft?.remaining??"—"}/></section>
  <section className="route-start"><h2>Start Fast Track</h2><p>Intentional verification only. This is not a compulsory daily target.</p><div className="route-sizes">{sizes.map(n=><button key={n} className={`btn ghost ${size===n?"active":""}`} onClick={()=>setSize(n)}>{n}</button>)}</div><button className="btn primary full-width" disabled={!ft?.readyToVerify||localSafe} onClick={()=>setRunning(true)}>Start {size}</button>{localSafe&&<p className="route-safe-note">Local Safe is active: verification writes are disabled against production data.</p>}<button className="btn ghost full-width" onClick={()=>setDetails(x=>!x)}>{details?"Hide details":"View"}</button></section>
  {details&&<section className="route-details"><div className="section-title-line"><h2>Fast Track Details</h2><Link className="btn ghost mini" href="/english/route-view?route=fast_track">All questions</Link></div><div className="route-table"><div className="route-table-head"><span>Origin</span><span>Total</span><span>Mastered</span><span>Remaining</span></div>{requiredOrigins.map(origin=>{const row=byOrigin.get(origin)||{origin,total:0,mastered:0,remaining:0};return <Link className="route-table-row" key={origin} href={`/english/route-view?route=fast_track&origin=${encodeURIComponent(origin)}`}><b>{origin}</b><span>{row.total}</span><span>{row.mastered}</span><span>{row.remaining} ›</span></Link>})}<div className="route-table-row route-table-total"><b>TOTAL</b><span>{ft?.total??0}</span><span>{ft?.mastered??0}</span><span>{ft?.remaining??0}</span></div></div></section>}
 </section>;
}
