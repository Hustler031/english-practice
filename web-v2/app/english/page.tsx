...
export default function EnglishHome() {
 ...
 return <>
  ...
  <section className="daily-hero"><div className="daily-hero-top"><div><span className="eyebrow">Day {dayNo} · Today</span><h1>Day {dayNo} · Today&apos;s English Practice</h1></div><span className="today-badge">{data?`${completed} done`:"Syncing"}</span></div><p>Finish your daily revision first, then use focused practice.</p><div className="goal-line"><span>Today&apos;s Goal</span><strong>{data?`${completed} / ${total}`:"—"}</strong></div><div className="progress-track"><i style={{width:`${percent}%`}}/></div><div className="daily-status">{data?(dailyComplete?"Daily practice complete for the current due clock.":`${actionableRemaining} actionable questions remain.`):"Using your last cached plan while fresh data syncs…"}</div><Link className="btn daily-button" href="/english/daily">{dailyComplete?"Daily Complete ✓":completed?"Continue Daily Practice":"Start Daily Practice"}</Link></section>
