...
export default function EnglishHome() {
  ...
  useEffect(()=>{if(ready){Promise.all([rpc<Summary>("english_dashboard_summary"),rpc<PhrasalHub>("english_get_phrasal_hub"),rpc<BankHub>("english_get_bank_coverage_hub"),rpc<SavedHub>("english_get_saved_revision_hub")]).then(([d,p,b,s])=>{setData(d);setPhrasal(p);setBank(b);setSaved(s);}).catch((e:any)=>setError(e.message));setPaused(readPausedQuiz());}},[ready]);
  ...
  const status=(accent:string)=>accent==="saved"?(saved?`${saved.stats.due} due`:"…"):accent==="phrasal"?(phrasal?(phrasal.today.ready?`${phrasal.today.count} today`:`${phrasal.stats.due} due`):"…"):accent==="starred"?(data?`${data.starred}`:"…"):accent==="bank"?(bank?`${bank.coverage.toFixed(1)}%`:"…"):"Today";
