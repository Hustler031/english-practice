...
function Rows({rows}:{rows:readonly (readonly [string,string,string])[]}){return <div className="legacy-list">{rows.map(([name,sub,href])=><Link className="legacy-row" href={href} key={href}><span className="legacy-row-copy"><b>{name}</b><small>{sub}</small></span></Link>)}</div>}

export default function LibraryHome(){const ready=useAuthGuard();if(!ready)return <EnglishLoading text="Checking session…"/>;return <div className="top-level-parity learner-rebuild-page"><section className="page-intro learner-page-header"><h1>Library</h1><p>Your own learning first, then reading and source history.</p></section><div className="library-groups"><section className="library-group"><div className="section-cap">My Learning</div><Rows rows={personal}/></section><section className="library-group"><div className="section-cap">Reading & Sources</div><Rows rows={sources}/></section></div></div>}
