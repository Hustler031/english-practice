...
const KEY="revision:v2:gk:fresh-session-history:v1";
...
function read(){if(!browser())return[];try{return clean(JSON.parse(localStorage.getItem(KEY)||"[]"));}catch{return[];}}
function write(rows:SessionRow[]){if(!browser())return;try{localStorage.setItem(KEY,JSON.stringify(rows.slice(0,MAX_ROWS)));}catch{}}
...
function recentIds(lane:string){return [...new Set(read().filter(x=>x.lane===lane).slice(0,MAX_RECENT_SESSIONS).flatMap(x=>x.ids))].slice(0,MAX_EXCLUDE);}
...
export async function loadFreshGkQuestions({name,args,requested,load}:{name:string;args:Args;requested:number;load:(expandedCount:number)=>Promise<GkQuestion[]>}){
 ...
 const lane=gkFreshLane(name,args),excluded=recentIds(lane),excludedSet=new Set(excluded);
 ...
 if(chosenIds.length)write([{lane,ids:chosenIds,at:Date.now()},...read()]);
 return chosen;
}

export function clearGkFreshSessionHistory(){if(!browser())return;try{localStorage.removeItem(KEY);}catch{}}
