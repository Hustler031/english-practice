...
const CACHE_PREFIX="revision:v2:gk:rpc-cache:";
const OUTBOX_KEY="revision:v2:gk:answer-outbox:v1";
const MAX_AGE=12*60*60*1000;
const BACKOFF=[1000,2500,5000,15000,30000,60000];
let running=false;let timer:ReturnType<typeof setTimeout>|null=null;let wired=false;
const browser=()=>typeof window!=="undefined";
const stable=(a?:Args)=>Object.keys(a||{}).sort().reduce<Args>((o,k)=>(o[k]=(a||{})[k],o),{});
const stableText=(a?:Args)=>JSON.stringify(stable(a));
const key=(n:string,a?:Args)=>`${CACHE_PREFIX}${n}:${stableText(a)}`;
...
function readCache<T>(name:string,args?:Args){if(!browser())return undefined;try{const raw=localStorage.getItem(key(name,args));if(!raw)return undefined;const e=JSON.parse(raw) as CacheEntry<T>;if(!e||Date.now()-e.at>MAX_AGE)return undefined;return e.data;}catch{return undefined;}}
function writeCache<T>(name:string,args:Args|undefined,data:T){if(!browser())return;try{localStorage.setItem(key(name,args),JSON.stringify({at:Date.now(),data}));}catch{}}
...
export async function flushGkAnswerOutbox(){if(!browser()||running||!navigator.onLine)return;const rows=readOutbox();if(!rows.length)return;const now=Date.now();const item=rows.find(x=>x.nextAt<=now);if(!item){const next=Math.min(...rows.map(x=>x.nextAt||now+60000));schedule(Math.min(60000,Math.max(500,next-now)));return;}running=true;try{try{const result=await network("gk_submit_answer",item.args);writeOutbox(readOutbox().filter(x=>x.id!==item.id));try{window.dispatchEvent(new CustomEvent("revision:gk-answer-durable",{detail:{id:item.id,result}}));}catch{}}catch{const latest=readOutbox();const hit=latest.find(x=>x.id===item.id);if(hit){hit.tries++;hit.nextAt=Date.now()+BACKOFF[Math.min(BACKOFF.length-1,hit.tries-1)];writeOutbox(latest);}}}finally{running=false;if(readOutbox().length)schedule(250);}}
...
export async function gkRpc<T=unknown>(name:string,args?:Args):Promise<T>{wire();if(name==="gk_submit_answer"){const questionId=String(args?.p_question_id||"").trim();const selected=String(args?.p_selected_option||"").toUpperCase();const attemptId=String(args?.p_attempt_id||"").trim()||`gk-${questionId}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;const payload={...(args||{}),p_attempt_id:attemptId};if(!questionId||!["A","B","C","D"].includes(selected))return network<T>(name,payload);const rows=readOutbox();if(!rows.some(x=>x.id===attemptId))rows.push({id:attemptId,args:payload,tries:0,nextAt:0,queuedAt:Date.now()});writeOutbox(rows);schedule(0);return {ok:true,queued:true,durable:false,attemptId} as T;}if(cacheable(name)){const cached=readCache<T>(name,args);if(cached!==undefined){void network<T>(name,args).then(fresh=>{writeCache(name,args,fresh);publish(name,args,fresh)}).catch(()=>{});return cached;}const fresh=await network<T>(name,args);writeCache(name,args,fresh);return fresh;}return network<T>(name,args);}
...
export function clearGkPrivateCache(){if(!browser())return;for(let i=localStorage.length-1;i>=0;i--){const k=localStorage.key(i);if(k?.startsWith(CACHE_PREFIX)||k===OUTBOX_KEY)localStorage.removeItem(k);}}
