...
const auditFix=read("supabase/managed-migrations/20260831162500_gk_v2_final_audit_seen_and_taxonomy_fix.sql");
...
must(has(auditFix,"greatest(coalesce(e.exposure_count,0),case when coalesce(a.attempts,0)>0 then 1 else 0 end)","gk_get_new_practice_hub","join profile p on p.question_id=q.question_id","gk.canonical_subject(q.subject) subject"),"final audit must treat historical attempts as seen and canonicalise Progress subjects");
must(!auditFix.includes("delete from gk.attempts")&&!auditFix.includes("delete from gk.exposures")&&!auditFix.includes("delete from gk.sessions")&&!auditFix.includes("update gk.attempts")&&!auditFix.includes("update gk.exposures"),"final audit repair must not rewrite learner history");
...
for(const item of forbidden)must(![foundation,teacherSelector,sprintFix,auditFix,quiz,questionText,teacher,progress,sprint,layout,navCss,practiceCss].some(t=>t.includes(item)),`GK final files unexpectedly reference ${item}`);
