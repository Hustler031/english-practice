...
// Re-run all earlier stage contracts first. Stage 3 may never weaken Stage 1/2 guarantees.
execFileSync(process.execPath,[at('web-v2/scripts/validate-grammar-stage1.mjs')],{cwd:repoRoot,stdio:'inherit'});
...
for(const p of [deploymentPath,curriculumPath,aliasesPath,bridgePath,submittedPath])if(!fs.existsSync(at(p)))fail(`required file missing: ${p}`);

const d=JSON.parse(read(deploymentPath));
const c=JSON.parse(read(curriculumPath));
const aliases=JSON.parse(read(aliasesPath));
const bridge=read(bridgePath),submitted=read(submittedPath);
...
if(d.guards?.productionFeatureFlagInitiallyOff!==true)fail('Grammar must deploy disabled');
if(d.guards?.schedulerInitiallyOff!==true)fail('Grammar scheduler must remain disabled until smoke');
if(d.guards?.noGoogleRuntimeDependency!==true)fail('Google runtime dependency guard missing');
if(d.guards?.readOnlyBrowsingWritesEvidence!==false)fail('read-only evidence mutation guard broken');
if(d.guards?.canonicalReusePreservesQuestionId!==true)fail('canonical identity guard missing');
if(d.guards?.partialDailyPublicationAllowed!==false)fail('partial publication guard broken');
if(d.guards?.mathsOrGkChangesAllowed!==false)fail('Maths/GK boundary guard broken');

const expectedMigrations=[
 '20260909011500_english_grammar_preflight.sql',
 '20260909012000_english_grammar_intelligence_foundation.sql',
 '20260909012500_english_grammar_curriculum_sync.sql',
 '20260909013000_english_grammar_daily_pipeline.sql',
 '20260909013100_english_grammar_owner_resolution_fix.sql',
 '20260909013200_english_grammar_selector_alias_fix.sql',
 '20260909020000_english_grammar_world_read_model.sql',
];
...
// Transport/AI source contract: Grammar is a first-class OIDC lane, but no server-side generator is allowed.
...
// Alias seed must stay conservative.
...
// No partial/bogus frozen snapshot may be committed. Stage 3 syncs the live curated Sheet only after checksum validation.
...
// Stage 3 remains deployment-readiness only: no accidental production activation in source.
const foundation=read('supabase/migrations/20260909012000_english_grammar_intelligence_foundation.sql');
...
for(const p of changed){
 if(/(^|\/)(maths?|gk)(\/|[-_.])/i.test(p))fail(`Stage 3 crossed Maths/GK boundary: ${p}`);
}

console.log(JSON.stringify({
 ok:true,
 stage:'grammar-intelligence-stage3-readiness',
 ruleCount:c.ruleCount,
 chapters:Object.keys(c.chapters||{}).length,
 dailyTarget:c.dailyTarget,
 sprintAliases:aliases.aliases.length,
 migrationCount:expectedMigrations.length,
 transportBase:d.transport.baseBranch,
 transportBaseHead:d.transport.baseHead,
 activationLast:d.activationOrder?.at(-1),
