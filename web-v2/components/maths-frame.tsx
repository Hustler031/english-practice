...
export function MathsFrame({children}:{children:ReactNode}){
  ...
  return <div className="maths-app"><header className="maths-header"><Link href="/maths" className="maths-brand"><strong>Maths Revision</strong><span>SSC Maths practice + learning intelligence</span></Link><div className="maths-header-actions">{safe&&<span className="maths-pill">Local Safe</span>}{pending>0&&<span className="maths-pill syncing"><i/>Syncing {pending}</span>}<Link className="maths-icon-btn" href="/" aria-label="Revision launcher" title="Revision launcher">⌂</Link><button className="maths-icon-btn" type="button" onClick={toggle} aria-label="Toggle theme">{theme==="dark"?"☀":"☾"}</button><button className={`maths-icon-btn ${refreshing?"spin":""}`} type="button" onClick={()=>void refresh()} aria-label="Refresh Maths" disabled={refreshing}>↻</button></div></header><main className="maths-content">{children}</main><nav className="maths-nav" aria-label="Maths navigation">{tabs.map(t=><Link key={t.id} href={t.href} className={`maths-nav-item ${active===t.id?"active":""}`} aria-current={active===t.id?"page":undefined}><b>{t.icon}</b><span>{t.label}</span></Link>)}</nav></div>;
}
