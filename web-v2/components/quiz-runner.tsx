...
type Props = { title:string; backHref:string; load:()=>Promise<Question[]>; module?:string; emptyText?:string; resumeSession?:PausedQuizSession|null; onPause?:(index:number,total:number)=>void|Promise<void>; onFinish?:()=>void|Promise<void> };
...
export default function QuizRunner({ title, backHref, load, module="practice", emptyText="No questions are available for this selection.", resumeSession, onPause, onFinish }: Props) {
  ...
  useEffect(()=>{if(loading)return;const onBack=()=>{void pause();};window.history.pushState({englishQuiz:true},"");window.addEventListener("popstate",onBack);return()=>window.removeEventListener("popstate",onBack);},[loading,backHref,idx,items.length,onPause]);
  ...
  async function pause(){try{await onPause?.(idx,items.length);}catch{}router.push(backHref);}
  ...
  async function finish(){try{await onFinish?.();}catch{}clearPausedQuiz();router.push(backHref);}

  if(loading)return <main className="center"><div className="muted">Loading {title}…</div></main>;
  if(!items.length)return <main className="shell quiz"><QuizTop title={title} onBack={()=>void pause()}/><div className="empty-state"><h2>{title}</h2><p className="muted">{error||emptyText}</p></div></main>;
  ...
  return <main className="shell quiz quiz-with-tools">
    <QuizTop title={title} onBack={()=>void pause()} count={`${idx+1} / ${items.length}`}/><div className="progress"><span style={{width:`${((idx+1)/items.length)*100}%`}}/></div>
