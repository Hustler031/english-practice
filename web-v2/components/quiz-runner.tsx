...
export default function QuizRunner({ title, backHref, load, module="practice", emptyText="No questions are available for this selection.", resumeSession, initialIndex=0, onPause, onFinish, onExit }: Props) {
  ...
  return <main className="shell quiz quiz-with-tools">
    ...
    <section className="quiz-card">
      ...
      {answer&&!recall&&<div className="result-wrap"><span className={`answer-cue ${answer.correct?"good-result":"bad-result"}`}>{answer.correct?"✓ Correct":"✕ Incorrect"}</span><Explanation q={q}/></div>}
...
function Explanation({q}:{q:Question}){const sections:Array<[string,string|undefined,string?]>=[["Explanation",q.explanation],["Example",q.example],["Usage",q.usageNote],["Tip",q.tip,"tip"],["Remember",q.memoryAid],["Related",q.related]];return <div className="explanation">{sections.filter(([,text])=>text).map(([heading,text,kind])=><div className={kind?"tipbox":""} key={heading}><h3>{heading}</h3><p><Emphasised text={text||""} q={q}/></p></div>)}</div>}
