...
function grouped(skills:CalcSkill[]){return buckets.map(label=>{const rows=skills.filter(x=>bucket(x.skill)===label);const total=rows.reduce((a,x)=>a+Number(x.total||0),0);const attempted=rows.reduce((a,x)=>a+Number(x.attempted||0),0);const accuracy=attempted?rows.reduce((a,x)=>a+Number(x.accuracy||0)*Number(x.attempted||0),0)/attempted:null;const measured=rows.filter(x=>Number(x.attempted||0)>0);const band=measured.length?[...measured].sort((a,b)=>bandRank(a.band)-bandRank(b.band))[0]?.band||"Needs work":"Not measured";return{label,total,attempted,accuracy,band};}).filter(x=>x.total>0);}
...
export default function MathsExamPreparationV2(){
  const ready=useAuthGuard();const router=useRouter();const[data,setData]=useState<Data|null>(null);const[error,setError]=useState("");const[busy,setBusy]=useState("");const[tab,setTab]=useState<Track>("academic");const[details,setDetails]=useState(false);const[info,setInfo]=useState(false);

  useEffect(()=>{try{const t=new URLSearchParams(window.location.search).get("tab");if(t==="calculation")setTab("calculation");}catch{}},[]);
  const switchTab=(next:Track)=>{setTab(next);try{const u=new URL(window.location.href);if(next==="calculation")u.searchParams.set("tab","calculation");else u.searchParams.delete("tab");window.history.replaceState(window.history.state,"",`${u.pathname}${u.search}${u.hash}`);}catch{}};
  ...
  return <section className="mex2-page">
    ...
    {tab==="academic"?<>
      ...
      <section className="mex2-repair"><div><strong>Targeted Repair</strong><button type="button" onClick={()=>setDetails(x=>!x)}>{details?"Less":"More"}</button></div>{details&&<div className="mex2-repair-grid">{reasons.filter(x=>x.id!=="CAL").map(r=><button key={r.id} type="button" disabled={!!busy||!!active} onClick={()=>void start("repair",r.id)}><b>{r.id}</b><span>{r.label}</span></button>)}</div>}</section>
    </>:<>
      ...
      <section className="mex2-repair"><div><strong>Skill diagnostics</strong><button type="button" onClick={()=>setDetails(x=>!x)}>{details?"Hide":"Show"}</button></div>{details&&<div className="mex2-skill-list">{calc.map(x=><div key={x.label}><span><b>{x.label}</b><small>{x.attempted?`${x.attempted} measured${x.accuracy==null?"":` · ${x.accuracy.toFixed(0)}%`}`:`${x.total} available`}</small></span><strong>{x.band}</strong></div>)}</div>}</section>
    </>}
