...
const REASONS: Reason[] = [
  ...
  { value: "explanation_weak", label: "Add / improve explanation" },
...
export default function QuestionRevisionActions({ questionId }: { questionId: string }) {
  ...
  async function submit(){
    ...
    try{
      if(reason==="correct_answer_doubtful"){
        ...
        setMessage("Sent for answer review. AI will explain the result; the canonical question will not be changed automatically.");
        ...
        setMessage(reason==="explanation_weak"?"Explanation request queued. AI will write a quality-checked explanation without changing the question or options.":"Improvement queued. Keep studying — it is checked in the background.");
      }
  ...
  async function useRevision(){
    ...
    try{await rpc("english_use_question_revision",{p_proposal_id:proposal.proposalId});setPreview(false);setMessage(proposal.feedbackReason==="explanation_weak"?"AI explanation is now active for your future practice.":"Revised version will be used in future practice.");await refresh();}
  ...
  return <div className="question-revision-actions">
    ...
    {proposal?.status==="ready"&&proposal.proposed?<div className="context-saved revision-ready-inline"><b>{proposal.feedbackReason==="explanation_weak"?"AI explanation ready":"Revision proposal ready"}</b><button className="btn ghost" type="button" onClick={()=>setPreview(true)}>{proposal.feedbackReason==="explanation_weak"?"Review explanation":"Preview"}</button></div>:null}
    {proposal?.status==="applied"?<div className="context-saved">{proposal.feedbackReason==="explanation_weak"?"AI explanation active for future practice.":"Revised version active for future practice."}</div>:null}
    ...
    {review?.status==="queued"||review?.status==="processing"?<div className="context-saved">{review.retryAt?"Answer review retry is scheduled. Nothing will be changed automatically.":"Correct-answer review queued. AI is checking it independently."}</div>:null}
    {review?.status==="reviewed"&&review.verdict==="valid"?<div className="context-saved"><b>Answer review: the current answer looks valid.</b>{review.rationale?` ${review.rationale}`:""}</div>:null}
    {review?.status==="reviewed"&&review.verdict==="issue_suspected"?<div className="error-box"><b>Answer review found a possible content issue.</b>{review.rationale?` ${review.rationale}`:""} The canonical question is still unchanged.</div>:null}
    ...
    {open&&<div className="ai-help-panel question-improve-sheet">
      ...
      {reason==="explanation_weak"&&<span>AI will add or improve only the explanation. The question, all four options and the correct answer stay unchanged.</span>}
      {reason==="correct_answer_doubtful"&&<span>AI independently checks the answer and explains its conclusion. It does not rewrite the canonical question.</span>}
    ...
    {preview&&proposal?.status==="ready"&&proposal.proposed&&<RevisionPreview payload={proposal.proposed} busy={busy} explanationOnly={proposal.feedbackReason==="explanation_weak"} onUse={()=>void useRevision()} onKeep={()=>void keepCurrent()} onClose={()=>setPreview(false)}/>} 
  </div>;
}

function RevisionPreview({payload,busy,explanationOnly,onUse,onKeep,onClose}:{payload:RevisionPayload;busy:boolean;explanationOnly:boolean;onUse:()=>void;onKeep:()=>void;onClose:()=>void}){
  ...
  const correctText=options.find(([key])=>key===payload.correctKey)?.[1]||"";
  return <div className="sheet-backdrop" role="dialog" aria-modal="true" aria-label="Revision proposal preview" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)onClose();}}>
    <section className="add-word-sheet intelligence-sheet">
      <div className="sheet-heading"><div><strong>{explanationOnly?"AI explanation ready":"Revision proposal ready"}</strong><span>Nothing changes until you choose {explanationOnly?"Use AI explanation":"Use revised version"}.</span></div><button className="control-icon" type="button" aria-label="Close" disabled={busy} onClick={onClose}>×</button></div>
      ...
      <div className="explanation"><h3>Correct answer: {correctText||"shown above"}</h3><p>{payload.explanation}</p></div>
      <div className="action-matrix"><button className="btn primary" type="button" disabled={busy} onClick={onUse}>{explanationOnly?"Use AI explanation":"Use revised version"}</button><button className="btn ghost" type="button" disabled={busy} onClick={onKeep}>Keep current</button></div>
    </section>
  </div>;
}
